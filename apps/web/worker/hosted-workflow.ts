...
> {
  async run(
    ...
    for (let observation = 0; observation < 290; observation += 1) {
      ...
      if (state === "SUCCEEDED") {
        const finalization = await step.do(`finalize span audio ${observation}`, async () => {
          const { reconcileHostedV209SpanWorkflowTerminal } = await import(
            "../src/server/hosted/hosted-v209-span-workflow-reconciliation"
          );
          return reconcileHostedV209SpanWorkflowTerminal(this.env, config, params);
        });
        if (finalization.state !== "FINALIZATION_PENDING") return { state };
      } else if (["FAILED", "CANCELLED", "EXPIRED"].includes(state)) return { state };
      if (observation < 289) {
