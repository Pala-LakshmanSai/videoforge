...
describe("ordinary V2-09 pair request materialization", () => {
  it("uses DB lane times, binds the exact SoulX batch hash, and CASes both bodies", async () => {
    const mageWork = [
      {
        taskId: "mage-task",
        outputPrefix:
          "tenant/account-a/workspace/workspace-a/project/project-a/revision/revision-a/lane/mage-image/job/mage_image-attempt",
    ...
    const soulxPrefix =
      "tenant/account-a/workspace/workspace-a/project/project-a/revision/revision-a/lane/soulx-avatar/job/soulx_avatar-attempt";
    const soulxWork = [
      {
        taskId: "soulx-task",
        ...
        spanAudioObjectKey:
          "tenant/account-a/workspace/workspace-a/project/project-a/revision/revision-a/lane/input/job/span-job/artifact/audio",
    ...
    const projections = {
      ...
      soulx_avatar: unsignedEnvelope("soulx_avatar", [
        "avatar-input",
        "span-input",
        "soulx-output",
      ]),
    ...
    const database = {
      transaction: vi.fn(async (fn: (tx: { query: typeof query }) => unknown) => fn({ query })),
    };
    const r2 = {
      sign: vi.fn(async (input: Record<string, unknown>) => ({
        url: `https://signed.example/${String(input.objectKey)}`,
        expiresAt: new Date(
          Date.parse(issuedAt) + Number(input.lifetimeSeconds) * 1000,
        ).toISOString(),
      })),
    ...
    const soulx = commits[1] as {
      envelope: Record<string, Record<string, unknown>>;
      batch: unknown;
    };
