...
export interface V213DatabaseWorkflowExecution {
  readonly schemaVersion: "videoforge.v213-database-acceptance-execution/v1";
  readonly checkpoint: "V2-10" | "V2-11" | "V2-12" | "V2-13";
  readonly workflowId: string;
  readonly workflowParams: Readonly<Record<string, unknown>>;
...
function exactExecution(value: Readonly<Record<string, unknown>>): V213DatabaseWorkflowExecution {
  if (
    value.schemaVersion !== "videoforge.v213-database-acceptance-execution/v1" ||
    ...
    Object.keys(value).sort().join(",") !==
      "call,checkpoint,deadlineAt,pollIntervalMs,schemaVersion,workflowId,workflowParams" ||
...
function workflowOutput(value: unknown): WorkflowOutput | null {
  ...
  if (
    !output.rawEvidence ||
    typeof output.rawEvidence !== "object" ||
    !output.receipt ||
    typeof output.receipt !== "object" ||
    !output.cleanup ||
    typeof output.cleanup !== "object" ||
...
function patchArtifactHash(
  ...
  return Object.freeze({
    ...document,
    [field]: canonicalSha256({ artifactSha256: referenceSha256 }),
  });
}
...
export class V213WorkflowLiveTransport implements V213LiveTransport {
  ...
  constructor(
    ...
    private readonly evidence: Pick<V213SqlSignedEvidenceStore, "signAndStore">,
  ...
  async execute(request: V213LiveExecutionRequest) {
    ...
    const deadline = Date.parse(this.execution.deadlineAt);
    ...
    const rawEvidence =
      request.checkpoint === "V2-13"
        ...
        : await (() => {
            ...
            return this.evidence.signAndStore(
              request.checkpoint === "V2-10"
                ? "V210_OUTPUT"
                : request.checkpoint === "V2-11"
                  ? "V211_EVIDENCE"
                  : "V212_OUTPUT",
              patchArtifactHash(output.rawEvidence, "canonicalEvidenceSha256", artifactSha256),
              artifactSha256,
            );
          })();
    const receiptReferenceSha256 = canonicalSha256({ workflowId, kind: "receipt" });
    const receipt = {
      ...patchArtifactHash(output.receipt, "canonicalArtifactSha256", receiptReferenceSha256),
...
export function createV213WorkerLiveAcceptanceExecute(
  ...
  return async (checkpoint, databaseValue, mode) => {
    const execution = exactExecution(databaseValue);
    if (execution.checkpoint !== checkpoint || !environment.HOSTED_PAIR_WORKFLOW)
