...
function parseDocument(value: unknown): StartDocument | null {
  ...
  if (
    !exactKeys(doc, ["schemaVersion", "workflowId", "requestSha256", "outerStateSha256", "params"])
  )
    return null;
  ...
  if (!exactKeys(params, ["accountId", "workspaceId", "generationRequestId", "cancelAt", "stopAt"]))
    return null;
  if (
...
function result(
  doc: Pick<StartDocument, "workflowId" | "requestSha256" | "outerStateSha256">,
...
export async function handleV213OperatorWorkflowStart(
  ...
  if (
    !document ||
    request.headers.get("x-videoforge-request-sha256") !== document.requestSha256 ||
  ...
  if (request.method === "POST") {
    ...
    if (computed !== document.requestSha256)
      return response({ error: { code: "V213_OPERATOR_WORKFLOW_REQUEST_DRIFT" } }, 409);
  }
  ...
  const pool =
    ...
    (() => {
      ...
      return {
        query: (sql: string, values: readonly unknown[]) => instance.query(sql, [...values]),
        close: () => instance.end(),
      };
    })();
  ...
  try {
    const claim =
      request.method === "POST"
        ? await pool.query(
            "SELECT public.videoforge_claim_v213_workflow_start($1::jsonb) AS claim",
            [
              JSON.stringify({
                tokenSha256,
                workflowId: document.workflowId,
                generationRequestId: document.params.generationRequestId,
        ...
        : await pool.query(
            "SELECT public.videoforge_load_v213_workflow_start($1::jsonb) AS claim",
            [
              JSON.stringify({
                tokenSha256,
                workflowId: document.workflowId,
    ...
    if (!workflow)
      return response({ error: { code: "HOSTED_PAIR_WORKFLOW_BINDING_MISSING" } }, 503);
    ...
    const completion = await pool.query(
      "SELECT public.videoforge_complete_v213_workflow_start($1::jsonb) AS result",
      [
        JSON.stringify({
          tokenSha256,
          workflowId: document.workflowId,
