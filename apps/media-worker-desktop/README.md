...
This package turns the provider-free `workers/media-local` ASR/span-audio/render core into install-only desktop
artifacts:
...
`publish_release=true` requires the exact new `media-worker-v0.1.27` tag and the protected release
...
This checkout prepares `media-worker-v0.1.27` to carry the Fal FlashHead render profile. It is not
...
.\windows-native-acceptance.ps1 `
  -InstallerPath .\VideoForge-Worker-0.1.27-Setup.exe `
