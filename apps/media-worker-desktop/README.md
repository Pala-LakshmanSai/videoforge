...
`publish_release=true` requires the exact new `media-worker-v0.1.31` tag and the protected release
...
This checkout prepares `media-worker-v0.1.31` to restore smooth scene-image zoom. It is not
...
.\windows-native-acceptance.ps1 `
  -InstallerPath .\VideoForge-Worker-0.1.31-Setup.exe `
