...
`publish_release=true` requires the exact new `media-worker-v0.1.20` tag and the protected release
...
next candidate is `media-worker-v0.1.20`; recompute the execution-bundle hash and publish
...
.\windows-native-acceptance.ps1 `
  -InstallerPath .\VideoForge-Worker-0.1.20-Setup.exe `
