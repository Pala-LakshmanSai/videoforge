...
`publish_release=true` requires the exact new `media-worker-v0.1.28` tag and the protected release
...
This checkout prepares `media-worker-v0.1.28` to carry the Fal FlashHead render profile. It is not
...
.\windows-native-acceptance.ps1 `
  -InstallerPath .\VideoForge-Worker-0.1.28-Setup.exe `
