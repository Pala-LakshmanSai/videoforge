...
`publish_release=true` requires the exact new `media-worker-v0.1.9` tag and the protected release
...
recorded repaired artifact is `media-worker-v0.1.9`; recompute the execution-bundle hash and publish
...
.\windows-native-acceptance.ps1 `
  -InstallerPath .\VideoForge-Worker-0.1.9-Setup.exe `
