 ...
 *   - the exact commit/tree is prepared and independently validated elsewhere;
 *   - this module may only move the default branch to that exact commit, either by
 *     a normal fast-forward ref update or by merging an already-open protected-branch
 *     pull request;
...
export const AUTHORITY_SCHEMA = "videoforge.v2-13-default-branch-workflow-repair-authority/v1";
export const VALIDATION_SCHEMA = "videoforge.v2-13-default-branch-workflow-repair-validation/v1";
export const RESULT_SCHEMA = "videoforge.v2-13-default-branch-workflow-repair-result/v1";
...
const PROPOSAL_SCHEMA = "videoforge.v2-13-default-branch-workflow-repair-proposal/v1";
const APPROVAL_SCHEMA = "videoforge.v2-13-default-branch-workflow-repair-approval/v1";
...
function assertTarget(target) {
  ...
  if (resulting.commit_sha === null) {
    if (resulting.parent_commit_shas !== null) fail("AUTHORITY_INVALID");
  } else {
...
export function validateRepairAuthority(authority, { now, skipTime = false } = {}) {
  ...
  if (
    ...
    authority.operation.action !== "FAST_FORWARD_OR_PROTECTED_BRANCH_WORKFLOW_REPAIR" ||
...
async function readExactRemoteState({ target, mechanism, runCommand, phase }) {
  ...
  assertWorkflowContent(sourceContent, target, target.release_source_commit);
  ...
  return { repository, ref, sourceContent, resultingCommit, resultingTree };
}

async function readFinalRemoteState({ target, runCommand, resultingCommitSha, expectedParents }) {
...
export async function executeDefaultBranchWorkflowRepair({
  ...
  try {
    ...
    const expectedParents =
      consumed.mechanism.type === "FAST_FORWARD"
        ? target.resulting.parent_commit_shas
