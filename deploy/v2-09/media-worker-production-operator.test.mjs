...
function fixture({ macosBytes } = {}) {
  ...
  const manifest = sortObject({
    ...
    windows: {
      url: "https://github.com/Pala-LakshmanSai/videoforge/releases/download/media-worker-v0.1.17/VideoForge-Worker-0.1.17-Setup.exe",
      sha256: windowsSha256,
      size_bytes: 225_000_000,
      trust: "UNSIGNED_BETA",
    },
    macos: {
      url: "https://github.com/Pala-LakshmanSai/videoforge/releases/download/media-worker-v0.1.17/VideoForge-Worker-0.1.17.dmg",
  ...
  const asset = (name, size, digest, contentType = "application/octet-stream") => ({
    ...
    browser_download_url: `https://github.com/Pala-LakshmanSai/videoforge/releases/download/media-worker-v0.1.17/${name}`,
  });
  const release = {
    tag_name: "media-worker-v0.1.17",
    target_commitish: sourceCommit,
    html_url: "https://github.com/Pala-LakshmanSai/videoforge/releases/tag/media-worker-v0.1.17",
    draft: false,
    prerelease: false,
    immutable: true,
    assets: [
      asset("VideoForge-Worker-0.1.17-Setup.exe", 225_000_000, windowsSha256),
      asset("VideoForge-Worker-0.1.17.dmg", effectiveMacosSize, effectiveMacosSha256),
      asset(
...
function sandbox() {
  ...
  const configuration = {
    ...
    releaseTag: "media-worker-v0.1.17",
    version: "0.1.17",
    sourceCommit,
...
function fetchFixture({ releaseStatus = 200, release, manifestBytes }) {
  const calls = [];
  const api =
    "https://api.github.com/repos/Pala-LakshmanSai/videoforge/releases/tags/media-worker-v0.1.17";
  const manifestUrl =
    "https://github.com/Pala-LakshmanSai/videoforge/releases/download/media-worker-v0.1.17/media-worker-release.json";
...
test("readback verifies every immutable asset and atomically materializes the exact manifest", async () => {
  ...
  try {
    ...
    assert.deepEqual(result, {
      ...
      operation_id: "readback-media-worker-0.1.17",
      release: "0.1.17",
      execution_bundle_sha256: executionBundleSha256,
      whisper_model_sha256: whisperModelSha256,
      release_manifest_sha256: releaseFixture.manifestSha256,
...
test("staged materialization dispatches once and derives a deterministic closed-world receipt", async () => {
  ...
  const bytesByName = new Map([
    ["VideoForge-Worker-0.1.17-Setup.exe", releaseFixture.windowsBytes],
    ["VideoForge-Worker-0.1.17.dmg", releaseFixture.macosBytes],
  ...
  for (const scenario of ["ambiguous", "tamper"]) {
    await t.test(scenario, async () => {
      ...
      const fetchImpl = async (url) => {
        ...
        const name = url.split("/").at(-1);
        const bytes =
          name === "VideoForge-Worker-0.1.17-Setup.exe"
            ? Buffer.from("tampered")
            : name === "VideoForge-Worker-0.1.17.dmg"
...
test("staged materialization streams large assets sequentially without arrayBuffer retention", async () => {
  ...
  const fetchImpl = async (url) => {
    ...
    const name = url.split("/").at(-1);
    if (name === "VideoForge-Worker-0.1.17-Setup.exe") return streamingResponse(0x61, largeSize);
    if (name === "VideoForge-Worker-0.1.17.dmg") return streamingResponse(0x62, largeSize);
...
test("macOS install verifies the exact DMG and universal2 app, preserves pairing, and proves ONLINE", async () => {
  ...
  try {
    ...
    assert.equal(result.release, "0.1.17");
