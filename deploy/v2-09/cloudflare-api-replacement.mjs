...
const fail = (code) => {
  throw Error("V2_09_API_REPLACEMENT_" + code);
};
const SCHEMA = "videoforge.v2-09-api-replacement-journal/v1";
export function createV209CloudflareApiReplacement(
  ...
  if (
    ...
    authority.execution !== "V2_09_KIE_FAL_API_REPLACEMENT_ONCE" ||
    authority.single_use !== true ||
    SECRET_NAMES.length !== 25 ||
    authority.production?.secret_count !== 25 ||
    authority.production?.secret_allowlist_sha256 !== hash([...SECRET_NAMES].sort()) ||
    !HASH.test(authority.production?.config_sha256) ||
    !HASH.test(authority.production?.worker_bundle_sha256) ||
    authority.caps?.max_incremental_usd !== 5 ||
    authority.caps?.max_completion_usd !== 5
  )
    fail("BINDING");
  ...
  const read = () => {
    ...
    try {
      ...
      if (j.schema_version !== SCHEMA || j.authority_sha256 !== authorityHash)
  ...
  return Object.freeze({
    verifyPredecessor: () =>
      locked(async () => {
        ...
        const j = {
          ...
          state: "CLAIMED",
          events: [],
          required_successor_secret_count: 25,
    ...
    deployOnce: () =>
      locked(async () => {
        ...
        try {
          ...
          j.events.push({ kind: "API_DEPLOY", status: "INTENT" });
          save(j);
          try {
            await ports.deploy(authority, artifact);
            j.events.push({ kind: "API_DEPLOY", status: "COMMITTED" });
            j.state = "DEPLOY_COMMITTED";
            save(j);
          } catch (error) {
            j.state = "DEPLOY_UNKNOWN";
            j.events.push({ kind: "API_DEPLOY", status: "UNKNOWN" });
            save(j);
            throw error;
          }
          const version = await ports.readback(authority, "DISABLED_UNQUALIFIED");
          j.state = "API_EFFECTIVE_VERIFIED";
          j.version = version;
          save(j);
          return version;
        } finally {
    ...
    containFailure: (cleanupAuthority) =>
      locked(async () => {
        if (
          !cleanupAuthority ||
          cleanupAuthority.source_commit !== authority.source_commit ||
          cleanupAuthority.production?.worker_name !== authority.production.worker_name ||
          cleanupAuthority.production?.config_sha256 !== authority.production.config_sha256 ||
          cleanupAuthority.production?.worker_bundle_sha256 !==
            authority.production.worker_bundle_sha256 ||
          cleanupAuthority.production?.secret_allowlist_sha256 !==
            authority.production.secret_allowlist_sha256 ||
          cleanupAuthority.production?.secret_count !== 25 ||
          cleanupAuthority.scope?.cleanup_only_recovery !== true ||
          cleanupAuthority.scope?.allow_redispatch !== false
        )
          fail("CLEANUP_BINDING");
        ports.assertCleanupAuthority(cleanupAuthority);
        const j = read();
        if (
          ![
            "DEPLOY_INTENT",
            "DEPLOY_UNKNOWN",
            "DEPLOY_COMMITTED",
            "API_EFFECTIVE_VERIFIED",
          ].includes(j.state)
        )
          fail("CLEANUP_REPLAY_OR_STATE");
        j.state = "DISABLE_INTENT";
        j.cleanup_authority_sha256 = hash(cleanupAuthority);
        j.events.push({ kind: "DISABLED_DEPLOY", status: "INTENT" });
        save(j);
        try {
          const version = await ports.disable(cleanupAuthority);
