...
function assertRejected(result, code) {
  ...
  assert.match(result.stdout + "\n" + result.stderr, /V2_13_CREDENTIAL_ROTATION_AUTHORITY_/u);
}

test("rotation/normalization authority is exact, single-use, and provider-free before execution", async () => {
...
test("rotation/normalization authority rejects order, scope, raw-output, and provider drift", async () => {
  ...
  assertRejected(
    await runMutation((values) => {
      values["approved-authority.json"].provider_free_recording.credentials_accessed = true;
    }),
    "AUTHORITY_RECORDING",
  );
  assertRejected(
