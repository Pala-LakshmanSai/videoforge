...
const fileNames = ["combined-credential-rotation-normalization-proposal.json", "validate-candidate.mjs"];
...
async function runMutation(mutate) {
  ...
  try {
    const values = {};
    const rawFiles = {};
    const originalProposal = JSON.parse(await readFile(path.join(sourceDirectory, fileNames[0]), "utf8"));
    ...
    mutate(values);
    const proposalChanged = JSON.stringify(values[fileNames[0]]) !== JSON.stringify(originalProposal);
...
test("rotation/normalization proposal is sealed, narrow, and provider-free", async () => {
  ...
  assert.equal(output.prior_authority_id, "v2-13-credential-bootstrap-reuse-20260827-082652z-90d6b19d");
...
test("rotation/normalization proposal rejects scope, ordering, and approval drift", async () => {
  assertRejected(
    await runMutation((values) => {
      values["combined-credential-rotation-normalization-proposal.json"].authority.google_secret_rotation_authorized = true;
    }),
    "AUTHORITY_google_secret_rotation_authorized",
  );
  assertRejected(
    await runMutation((values) => {
      values["combined-credential-rotation-normalization-proposal.json"].exact_execution_graph.operations.reverse();
    }),
    "GRAPH_ORDER",
  );
  assertRejected(
    await runMutation((values) => {
      values["combined-credential-rotation-normalization-proposal.json"].requested_scope.google.raw_new_secret_stdout_authorized = true;
    }),
    "GOOGLE_raw_new_secret_stdout_authorized",
  );
  assertRejected(
    await runMutation((values) => {
      values["combined-credential-rotation-normalization-proposal.json"].requested_scope.local_protected_storage.r2_provider_mutation_authorized = true;
    }),
    "STORAGE_R2_PROVIDER",
  );
  assertRejected(
    await runMutation((values) => {
      values["combined-credential-rotation-normalization-proposal.json"].approval_request.requested_exact_action = "Approve one single-use execution.";
    }),
...
test("rotation/normalization proposal rejects receipt and source drift", async () => {
  ...
  assertRejected(
    await runMutation((values) => {
      values["combined-credential-rotation-normalization-proposal.json"].source.source_contract_hashes["apps/web/src/server/hosted/auth.ts"] = `sha256:${"0".repeat(64)}`;
    }),
    "SOURCE_BINDING_apps/web/src/server/hosted/auth.ts",
  );
  assertRejected(
    await runMutation((values) => {
      values["combined-credential-rotation-normalization-proposal.json"].stop_conditions.no_retry = false;
    }),
    "STOP_no_retry",
  );
});
