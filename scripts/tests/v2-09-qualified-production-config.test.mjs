...
test("release validator binds exact bytes and admitted immutable media worker versions", () => {
  ...
  const successor = JSON.parse(JSON.stringify(releaseManifest()).replaceAll("0.1.17", "0.1.22"));
  const successorBytes = Buffer.from(JSON.stringify(successor));
  assert.equal(validateReleaseManifest(successorBytes, sha256(successorBytes)).version, "0.1.22");
