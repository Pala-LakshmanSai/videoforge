...
3. Preserve current HEAD, newer commits, committed migrations, private inputs, accepted outputs, the exact active model artifacts, the two retained model volumes, and evidence required by active foundations or gates; never reset or reinterpret them. Only V2 task briefs belong in the working tree; Git history records removed planning files.
...
reconcile the repository sources; do not guess from prior chat text.
