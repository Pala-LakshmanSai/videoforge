...
After approval, deploy only the approved staging resources and publish only approved signed installers. Prove two real invited accounts with isolated DB rows/objects/devices/leases, auth/admission negatives, migration/backup/restore, signed large transfer, installed Windows and macOS owned ASR/render, autostart/offline/reconnect/restart/replay/cancel/timeout/stale-result behavior, durable hashes/probes, secret isolation, and real-Chrome staging. Record immutable deployment/release/config identities, timings, `$0` provider CPU truth plus user-device and remaining provider costs, settled finite cost, ongoing charges, rollback, and zero RunPod workers. Update evidence/gates/CURRENT_STATE, commit, and stop before V2-07.
```
...
Verify bounded checkpoint-specific authority and cap, exact approved versus actual resources/rates/recurring charges, tenant-isolated Better Auth/Neon/R2/devices/leases, least-privilege secrets, hosted Cloudflare orchestration, signed Windows/macOS installer identity and personal-worker ASR/render parity, autostart/offline/reconnect/stale-result/cancel/backup/restore, explicit Delete-to-R2 behavior, Chrome proof, and RunPod-disabled/zero-worker truth. Local fixtures or unsigned packages cannot prove deployment/release.
