...
```text
Work on VideoForge checkpoint V2-06 only: hosted auth, Neon, R2, Cloudflare orchestration, and signed tenant-owned Windows/macOS personal media workers.
...
This request authorizes provider-free activation, bounded local application/infrastructure/context work, local tests/builds, and narrowly scoped read-only identity/quota/region/current-rate lookups through already configured Cloudflare, Neon, Google OAuth, and signing-provider credentials at a $0 cap. Do not print secrets. No resource/config/secret/OAuth/DNS mutation, deployment, installer publication/signing/notarization, email delivery, paid request, retention change, or spend is authorized yet. RunPod calls are not authorized.

First finish production adapters/manifests for Cloudflare-hosted app/API and durable orchestration, Better Auth Google plus atomic invite admission, Neon PostgreSQL, private tenant R2, and account-owned Windows/macOS personal workers containing pinned whisper.cpp and FFmpeg/FFprobe. Implement zero-configuration installer UX, one-confirmation browser pairing, OS-keychain device credentials, tenant-bound leases, fresh signed artifact ports, streaming uploads, migrations/rollback, heartbeat/reconnect/restart/replay/cancel/stale-result fencing, explicit user Delete-to-R2 semantics, backups, observability, and exact Windows/macOS parity. Keep GPU transport fake and disabled.
...
After approval, deploy only the approved staging resources and publish only the approved immutable ImageForge-style beta installers with exact trust metadata. Prove two real invited accounts with isolated DB rows/objects/devices/leases, auth/admission negatives, migration/backup/restore, signed large transfer, installed Windows and macOS owned ASR/render, autostart/offline/reconnect/restart/replay/cancel/timeout/stale-result behavior, durable hashes/probes, secret isolation, and real-Chrome staging. Record immutable deployment/release/config identities, observed OS warning behavior, timings, `$0` provider CPU truth plus user-device and remaining provider costs, settled finite cost, ongoing charges, rollback, and zero RunPod workers. Update evidence/gates/CURRENT_STATE, commit, and stop before V2-07.
```
...
Verify bounded checkpoint-specific authority and cap, exact approved versus actual resources/rates/recurring charges, tenant-isolated Better Auth/Neon/R2/devices/leases, least-privilege secrets, hosted Cloudflare orchestration, immutable Windows/macOS installer identity with exact ImageForge-style beta trust metadata and personal-worker ASR/render parity, autostart/offline/reconnect/stale-result/cancel/backup/restore, explicit Delete-to-R2 behavior, Chrome proof, and RunPod-disabled/zero-worker truth. Local fixtures or unverified packages cannot prove deployment/release.
