...
const releaseSourceCommit = "e737eac44458a04c7de47a0f3f42d82cb9506d47";
const supersededProposalCommit = "e3bdabc161c60e5334c4055b5636b7fd768a86df";
const exactProposalCommitFiles = [
  "deploy/v2-13/validate-full-live-approval.mjs",
  ...
  "scripts/tests/v2-13-full-live-authority.test.mjs",
];
...
assert(proposal.sealing.sealed_for_exact_user_approval === true && proposal.sealing.current_bytes_are_approval_ineligible === false, "SEALED");
assert(proposal.source.release_source_commit === releaseSourceCommit && proposal.source.base_source_commit_before_semantic_tag_repair === undefined && proposal.source.proposal_record_commit === null && proposal.source.future_approval_record_commit === null, "SOURCE_LINEAGE");
...
for (const component of Object.values(exactReleaseComponents)) {
  const componentBytes = execFileSync("git", ["show", `${releaseSourceCommit}:${component.path}`], { cwd: root });
...
assert(ref.creation_requested === true && ref.exact_ref === "refs/tags/videoforge-v2-13-release-20260826-v3" && ref.exact_tag_name === "videoforge-v2-13-release-20260826-v3" && ref.exact_target_commit === releaseSourceCommit && ref.tag_kind === "LIGHTWEIGHT" && ref.maximum_new_refs === 1, "EXACT_RELEASE_REF");
...
const proposalRecordCommit = execFileSync("git", ["rev-parse", "HEAD"], {
  cwd: root,
  encoding: "utf8",
}).trim();
assert(proposalRecordCommit !== releaseSourceCommit, "PROPOSAL_COMMIT_DISTINCT");
assert(
  execFileSync("git", ["rev-parse", "HEAD^"], { cwd: root, encoding: "utf8" }).trim() ===
    releaseSourceCommit,
  "PROPOSAL_PARENT_SOURCE",
);
assert(
  JSON.stringify(
    execFileSync("git", ["diff", "--name-only", `${releaseSourceCommit}..HEAD`], {
      cwd: root,
      encoding: "utf8",
    })
      .trim()
      .split("\n")
      .filter(Boolean),
  ) === JSON.stringify(exactProposalCommitFiles),
  "PROPOSAL_COMMIT_FILES",
);
assert(
...
console.log(JSON.stringify({ status: "PASS", proposal_sha256: expectedProposalSha256, release_source_commit: releaseSourceCommit, proposal_record_commit: proposalRecordCommit, authority: "ABSENT", exact_release_tag_requested: "videoforge-v2-13-release-20260826-v3", exact_database_roles: ["videoforge_hosted_runtime", "videoforge_hosted_reconciler"], external_calls: 0, mutations: 0, gpu_use: 0, spend_usd: 0 }));
