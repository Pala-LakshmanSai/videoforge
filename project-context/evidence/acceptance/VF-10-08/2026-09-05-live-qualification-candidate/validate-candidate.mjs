...
const expectedPrecursorSourceCommit = "4f2c5cb3c602ffb48db4a1dc7143477259202896";
...
check(sourceCommit === expectedPrecursorSourceCommit, "precursor source commit must equal final repair");
const priorAttempt = proposal.superseded_prior_publication_attempt;
check(priorAttempt?.prior_source_commit === "c3bcac0902c607d69ce101c21b081d4926b33bd8" && priorAttempt?.prior_approval_consumed === true && priorAttempt?.prior_approval_reusable === false, "prior approval is consumed and non-reusable");
check(priorAttempt?.workflow_run_id === 33961641686 && priorAttempt?.result === "FAILED_BEFORE_AUTHENTICATION_OR_PUBLICATION" && priorAttempt?.failure_reason === "TMPDIR_DIRECTORY_ABSENT", "prior publication run failure record");
check(priorAttempt?.publication_completed === false && priorAttempt?.immutable_digest_produced === false && priorAttempt?.image_artifact_produced === false && priorAttempt?.runpod_mutations === 0 && priorAttempt?.gpu_use === false && priorAttempt?.external_spend_usd === 0 && priorAttempt?.source_drift_invalidated_prior_approval === true && priorAttempt?.reuse_authorized === false, "prior publication run no-artifact/no-spend boundary");
const secondPriorAttempt = proposal.second_superseded_prior_publication_attempt;
check(secondPriorAttempt?.prior_source_commit === "67af7d9a7f6d2de7e8d7ff96e1a18f15ab78ecbd" && secondPriorAttempt?.prior_approval_consumed === true && secondPriorAttempt?.prior_approval_reusable === false, "second prior approval is consumed and non-reusable");
check(secondPriorAttempt?.workflow_run_id === 33962783745 && secondPriorAttempt?.result === "FAILED_BEFORE_AUTHENTICATION_OR_PUBLICATION" && secondPriorAttempt?.failure_reason === "LITERAL_BACKSLASH_IN_SINGLE_QUOTED_DOCKER_GO_TEMPLATE", "second prior publication run failure record");
check(secondPriorAttempt?.local_image_built === true && secondPriorAttempt?.publication_completed === false && secondPriorAttempt?.immutable_digest_produced === false && secondPriorAttempt?.registry_artifact_produced === false && secondPriorAttempt?.runpod_mutations === 0 && secondPriorAttempt?.gpu_use === false && secondPriorAttempt?.external_spend_usd === 0 && secondPriorAttempt?.source_drift_invalidated_prior_approval === true && secondPriorAttempt?.reuse_authorized === false, "second prior publication run no-artifact/no-spend boundary");
...
check(JSON.stringify(acceptance.superseded_prior_publication_attempt) === JSON.stringify(proposal.superseded_prior_publication_attempt), "acceptance prior publication attempt record");
check(JSON.stringify(acceptance.second_superseded_prior_publication_attempt) === JSON.stringify(proposal.second_superseded_prior_publication_attempt), "acceptance second prior publication attempt record");
...
same(acceptance.publication_workflow_sha256, proposal.control.publication_workflow.sha256, "acceptance publication workflow hash");
