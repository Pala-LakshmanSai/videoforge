...
const [proposalBytes, max1Bytes, max2Bytes, authorityBytes, attempt21Bytes, publicationBytes, stateBytes, gateBytes, taskBytes, startBytes, activationBytes, controlBytes, controlTestBytes, qualificationBytes, qualificationTestBytes] =
  await Promise.all([
...
assert(authority.schema_version === "videoforge.v2-07-attempt22-template-environment-readback-authority/v1", "authority_schema");
assert(authority.checkpoint === "V2-07" && authority.task_id === "VF-10-07" && authority.attempt === 22, "authority_scope");
...
assert(authority.lineage?.control_source_commit === EXPECTED.control && authority.lineage?.image_source_commit === EXPECTED.source, "authority_source");
...
assert(authority.lineage?.prior_proposal_sha256 === proposal.lineage?.prior_proposal_sha256 && authority.lineage?.prior_authority_sha256 === proposal.lineage?.prior_authority_sha256, "authority_prior_lineage");
...
assert(authority.execution_boundary?.image_republication_authorized === false && authority.execution_boundary?.v2_08_authorized === false, "authority_forbidden_boundary");
assert(authority.forbidden?.includes("V2-08 or successor work") && authority.forbidden?.includes("model download preparation quantization or volume mutation"), "authority_forbidden");
