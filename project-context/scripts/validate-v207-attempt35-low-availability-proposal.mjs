...
const expected = Object.freeze({
  ...
  acceptance: "sha256:8e5d58e520e49c147a139dc612bd347b6a01ef62a4ecde2cb47d11acfd6b55c6",
...
for (const key of ["proposal", "acceptance", "max1", "max2"]) assert(sha(bytes[key]) === expected[key], `${key.toUpperCase()}_HASH`);
...
assert(acceptance.attempt === 35 && acceptance.result === "PROVIDER_FREE_CANDIDATE_AWAITING_FRESH_EXACT_APPROVAL_AND_POSITIVE_NUMERIC_CAP" && acceptance.candidate?.proposal_sha256 === expected.proposal && acceptance.candidate?.authority_recorded === false && acceptance.provider_boundary?.authority_active === false && acceptance.provider_boundary?.external_spend_usd === 0, "ACCEPTANCE");
...
for (const value of [expected.proposal, expected.acceptance, expected.max1, expected.max2, expected.priorAuthority, expected.priorClosure, "V207_APPROVED_AUTHORITY_SHA256 = null", "V207_APPROVED_FINITE_CAP_USD: number | null = null", "V2-08"]) assert(context.includes(value), `CONTEXT_${value.slice(-8)}`);
process.stdout.write("V2-07 Attempt35 LOW-availability proposal validation PASS (provider-free; no authority/cap)\n");
