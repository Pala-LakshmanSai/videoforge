...
for (const value of [expected.proposal, expected.acceptance, expected.authority, expected.max1, expected.max2, expected.priorAuthority, expected.priorClosure, "no_live_authority_attempt35_consumed", "V2-08"]) assert(context.includes(value), `CONTEXT_${value.slice(-8)}`);
const exactAttempt37SuccessorActive =
  context.includes("V207_APPROVED_FINITE_CAP_USD: number | null = 4") &&
  context.includes("sha256:6ff97af22dd025e9298a830a9bcd946f18fe376745f39ed6e5c15b791e3f390e") &&
  context.includes("sha256:812899db3d2225224ea231112d2eba150ffbbd254148e71f94c81a44de32cadf") &&
  context.includes("attempt37_bounded_mutation_authorized");
assert(context.includes("V207_APPROVED_FINITE_CAP_USD: number | null = null") || exactAttempt37SuccessorActive, "CONTEXT_ACTIVE_AUTHORITY_LIFECYCLE");
