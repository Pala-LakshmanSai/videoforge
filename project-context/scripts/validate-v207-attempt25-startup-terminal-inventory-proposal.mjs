...
const paths = {
  ...
  closure: resolve(root, "project-context/evidence/acceptance/VF-10-07/2026-08-21-live-qualification/failed-attempt-24.json"),
...
const EXPECTED = {
  ...
  acceptance: "sha256:12fe5d7281cb82a6ee1d7766c474bdfafac990c204906750afaa205a7ba1d633",
...
assert(lineage?.failed_attempt_evidence_sha256 === EXPECTED.closure && lineage?.prior_proposal_sha256 === "sha256:be17430ce61a48a823a1ac87a128e83e44cfb88b01163331c285280e95274137" && lineage?.prior_authority_sha256 === "sha256:fccd60a68ee93f522d9e378012c5ccbefb182f6b03e26fde1b5940506ab9c412", "prior_lineage");
...
assert(closure.schema_version === "videoforge.v2-07-live-failed-attempt/v1" && closure.attempt === 24 && closure.qualification_boundaries?.v2_07 === "NOT_QUALIFIED", "prior_closure_scope");
assert(closure.authority_proposal_sha256 === "sha256:be17430ce61a48a823a1ac87a128e83e44cfb88b01163331c285280e95274137" && closure.approved_authority?.sha256 === "sha256:fccd60a68ee93f522d9e378012c5ccbefb182f6b03e26fde1b5940506ab9c412" && closure.billing?.maximum_cumulative_finite_spend_usd === 4, "prior_closure_authority");
assert(closure.failure?.code === "RUNPOD_QUIESCENT_NOT_CONFIRMED" && closure.failure?.job_dispatch_reached === false && closure.failure?.gpu_jobs_submitted === 0 && closure.failure?.batch_count === 0, "prior_closure_failure");
...
assert(acceptance.candidate?.proposal_sha256 === EXPECTED.proposal && acceptance.candidate?.max1_sha256 === EXPECTED.max1 && acceptance.candidate?.max2_sha256 === EXPECTED.max2 && acceptance.candidate?.control_source_commit === EXPECTED.control && acceptance.candidate?.prior_attempt24_closure_sha256 === EXPECTED.closure && acceptance.candidate?.maximum_cumulative_finite_spend_usd === null && acceptance.candidate?.fresh_numeric_cap_required === true && acceptance.candidate?.authority_path === "approved-authority.json" && acceptance.candidate?.authority_sha256 === EXPECTED.authority && acceptance.candidate?.provider_calls_authorized === false && acceptance.candidate?.provider_mutations_authorized === false && acceptance.candidate?.gpu_use_authorized === false, "acceptance_binding");
...
assert(state.includes("phase: serverless_v2_v2_07_attempt25_closed") && state.includes("maximum_external_spend_usd: 0"), "state_phase");
assert(state.includes(candidatePath) && state.includes(EXPECTED.proposal) && state.includes(EXPECTED.control) && state.includes("provider_calls_authorized: false") && state.includes("current_authority: null") && state.includes(EXPECTED.authority), "state_pointer");
assert(gates.includes("latest_closed_proposal: \"" + candidatePath + "\"") && gates.includes("latest_closed_proposal_sha256: \"" + EXPECTED.proposal + "\"") && gates.includes("authority_mode: none_attempt25_consumed") && gates.includes("closed_authority: \"evidence/acceptance/VF-10-07/2026-08-21-attempt25-startup-terminal-inventory-candidate/approved-authority.json\"") && gates.includes(EXPECTED.authority) && gates.includes("pending_numeric_cap_usd: null"), "gate_pointer");
...
assert(activation.includes("V207_PENDING_PROPOSAL_SHA256") && activation.includes(EXPECTED.proposal) && activation.includes("V207_APPROVED_FINITE_CAP_USD: number | null = null"), "activation_approved");
