...
assert((preExecution && gates.includes("authority_mode: attempt26_bounded_mutation_authorized") && gates.includes("pending_numeric_cap_usd: 4")) || (closed && gates.includes(`latest_closed_proposal_sha256: \"${EXPECTED.proposal}\"`) && gates.includes(`latest_closed_authority_sha256: \"${EXPECTED.authority}\"`) && gates.includes(`closure_evidence_sha256: \"${EXPECTED.closure26}\"`) && gates.includes("authority_mode: none_attempt26_consumed") && gates.includes("pending_numeric_cap_usd: null")), "gate_authority_pointer");
...
process.stdout.write(`V2-07 Attempt26 FINALIZE transport validation PASS (${EXPECTED.proposal}; authority ${EXPECTED.authority}; closure ${EXPECTED.closure26}; ${closed ? "consumed" : "pre-execution"})\n`);
