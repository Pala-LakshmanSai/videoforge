...
| `DEC_UX_002` | Use a medium-scale minimal production-console UI with an 18 px root, 52 px normal controls, a full-width internally inset active-project command bar, restrained project/progress hierarchy, strong vertical pipeline, live artifact panel, and the approved fixed-base floating dock with strong fine-pointer proximity magnification, an expanding backing tile including the active route, outward neighbor motion, and reduced-motion neutrality. Ordinary user-facing text remains at least 16 px; the accepted fixture shell retains only its existing 14.4–15.3 px developer-only health/fixture/terse technical metadata exception. | User rejected the first shell as too small/zoomed-out, dense, and explanatory; corrected the narrow centered top bar; requested a smooth macOS-like dock; then rejected the 20 px/60 px application scale as oversized while explicitly freezing the dock size and later asking the hovered icon plus active backing tile to expand on 2026-08-09. The user accepted the resulting shell again on 2026-08-09 before implementation resumed. |
