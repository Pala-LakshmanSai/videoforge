...
| `DEC_AVATAR_008` | EchoMimicV3-Flash uses a VideoForge-prepared FP8 runtime derived from the pinned first-party Flash weights with pinned TorchAO; it receives only scheduled short voiceover spans and does not use Long Video CFG | On 2026-08-13 the user retained Echo FP8 but explicitly rejected the prior long-video architecture. The prepared runtime must avoid the uncarded third-party pickle, preserve the pinned source/weights/base/audio-encoder lineage, and pass the exact volume/model/GPU gate before production. Historical BF16 and FP8 attempts remain evidence, not approval of Long Video CFG or a production profile. |
...
| `DEC_RUNPOD_002` | Use two different persistent `EU-RO-1` model volumes and two disposable API-created Pods: one volume/Pod only for Mage INT8 ConvRot and one volume/Pod only for EchoMimic FP8. Never cross-mount or share the volumes. Derive each not-yet-approved volume capacity from its verified manifest plus explicit operational headroom. Prepare and checksum each model once; normal Pod boot must load offline from its volume. At project start refresh live compatible GPU inventory and let the user select the Mage and Echo GPUs independently. Start both Pods in parallel, delete each Pod after its lane finishes, and retain both volumes. | The user explicitly selected the working ImageForge lifecycle on 2026-08-13. Persistent-volume pricing is an accepted fixed cost and must not be optimized away. Pods are the billed runtime unit, so time to `model_ready` is minimized and measured: ideal target ≤2 minutes; the user's reported ImageForge 3–4-minute baseline is context, not a measured VideoForge result. |
...
| `GATE_IMAGE_001` | **OPEN:** the new exact Mage INT8 ConvRot/4-step/guidance-1/1280×720 persistent-volume profile has not passed the 40-prompt/300-image quality and lifecycle suite; earlier BF16 outputs do not qualify it | No Mage production profile |
...
| `GATE_GPU_001` | **OPEN:** no live-inventory selection matrix yet proves exact compatible GPU choices independently for Mage INT8 and Echo FP8, including selected-versus-actual GPU identity | No production Mage/Echo GPU choice can become selectable |
| `GATE_RUNPOD_001` | **OPEN:** the two-volume/two-disposable-Pod lifecycle is approved but unqualified; exact manifests, no cross-mount, offline normal boot, idempotent create/delete, and zero-Pod/two-retained-volume cleanup remain to be proven | Live RunPod production dispatch remains disabled |
| `GATE_COST_001` | **OPEN:** the fixed cost of both retained volumes is accepted, but representative Pod boot/load/inference costs and cold/warm model-ready timings are unmeasured | Marginal-cost estimates and the ≤2-minute model-ready SLO remain unverified |
...
- 30-minute default hard cap: $1.50, user-adjustable only up to the MVP contract ceiling of $2; replace historical Serverless ranges with measured disposable-Pod costs before production.
