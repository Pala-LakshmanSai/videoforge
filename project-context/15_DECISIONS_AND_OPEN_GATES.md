...
| `GATE_IMAGE_002` | **OPEN — USER-ACCEPTED RISK:** exact Hugging Face checkpoint metadata was inaccessible (HTTP 401/signed-out 404); Microsoft source labels Mage-Flow MIT but also gives research-only deployment guidance | Does not block the user-authorized locked-model qualification, but blocks any VideoForge claim that Mage checkpoint access/permission is unambiguous |
