...
| `DEC_UX_002` | Use a large-scale minimal production-console UI with 20/18 px desktop/mobile roots, 60 px controls, a full-width internally inset active-project command bar, oversized project/progress hierarchy, strong vertical pipeline, live artifact panel, and an approximately 20%-larger floating dock with fine-pointer proximity magnification and reduced-motion neutrality | User rejected the first shell as too small/zoomed-out, dense, and explanatory, corrected the later narrow centered top bar, then explicitly required a smooth macOS-like dock and larger components on 2026-08-09 |
