...
| `DEC_AVATAR_008` | After BF16 remained unavailable beyond twenty minutes, qualify the same 10.12-second EchoMimicV3-Flash sample using TorchAO FP8 dynamic activation and weight quantization plus upstream Long Video CFG on RTX 4090; retain exact inputs and output duration, then stop all paid resources | User explicitly selected FP8 on RTX 4090 or RTX 5090 on 2026-08-13. RTX 4090 is selected because the pinned runtime supports Ada SM89; RTX 5090 would require a separate CUDA/PyTorch upgrade. Runtime-created FP8 avoids the uncarded third-party pickle checkpoint. Paid dispatch still requires a fresh cap because delayed billing leaves only `$0.1519632055` under the prior `$2` cumulative ceiling. |
