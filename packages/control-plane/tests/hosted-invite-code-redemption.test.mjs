...
test("an exact retained 0046 ledger upgrades through 0049 and preserves admitted sessions", async () => {
  const database = new PGlite({ extensions: { pgcrypto } });
  try {
    await database.exec("CREATE EXTENSION IF NOT EXISTS pgcrypto");
    const executor = new PGliteExecutor(database);
    const sources = await loadMigrationSources();
    assert.equal(sources.at(-1)?.version, 49);
    ...
    assert.deepEqual(upgraded.appliedVersions, [47, 48, 49]);
    assert.deepEqual(
