...
test("an exact retained 0046 ledger upgrades only 0047 and preserves admitted sessions", async () => {
  const database = new PGlite({ extensions: { pgcrypto } });
  try {
    await database.exec("CREATE EXTENSION IF NOT EXISTS pgcrypto");
    const executor = new PGliteExecutor(database);
    const sources = await loadMigrationSources();
    assert.equal(sources.at(-1)?.version, 47);
    ...
    assert.deepEqual(upgraded.appliedVersions, [47]);
    assert.deepEqual(
