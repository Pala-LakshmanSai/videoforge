...
test("0203 adjudicates only the exact unresolved claimed batch and settles known provider cost", () => {
  assert.match(sql, /pg_get_functiondef\('public\.videoforge_fail_hosted_prompt_run\(uuid,text,text,boolean,bigint\)'::regprocedure\)/u);
  assert.match(sql, /supplied_additional_known_cost_micro_usd NOT BETWEEN 0 AND 250000/u);
  ...
  assert.match(sql, /supplied_known_cost_micro_usd NOT BETWEEN 0 AND 250000/u);
