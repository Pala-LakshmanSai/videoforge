...
const thirdAttempt = id(10);
const firstBundle = `sha256:${"a".repeat(64)}`;
const secondBundle = `sha256:${"b".repeat(64)}`;
const thirdBundle = `sha256:${"c".repeat(64)}`;

test("0208 permits exactly one provider-free retry after the 0207 render process failure", async () => {
  const db = new PGlite();
  try {
    await db.exec(`
...
END; $$;
    ...
    assert.equal((await db.query(`SELECT has_function_privilege(
      'videoforge_v209_runtime_dc9612d6',
      'public.videoforge_prepare_hosted_api_second_render_process_recovery(uuid,uuid,uuid,uuid,uuid,uuid,text)',
    ...
    const call = (bundle, retryId = thirdAttempt) => db.query(
      `SELECT videoforge_prepare_hosted_api_render_recovery($1,$2,$3,$4,$5,$6,$7) AS value`,
      [account, workspace, user, project, secondAttempt, retryId, bundle]);
    ...
    const state = (await db.query(`SELECT request.state AS request_state,
      ...
      JOIN hosted_api_second_render_process_recoveries recovery
    ...
    assert.equal((await db.query(`SELECT count(*)::int AS count FROM hosted_cpu_job_attempts
      WHERE kind='RENDER'`)).rows[0].count, 2);
    assert.equal((await db.query(`SELECT detail->>'provider_actions_created' AS paid
      FROM video_runtime_events WHERE reason='LOCAL_RENDER_PROCESS_RECOVERY'`)).rows[0].paid, "false");
    const replay = (await call(thirdBundle, id(12))).rows[0].value;
    assert.deepEqual([replay.retry_attempt_id, replay.replayed], [thirdAttempt, true]);
    await assert.rejects(call(firstBundle, id(12)), /identity drift/u);
    await assert.rejects(db.query(`SELECT videoforge_prepare_hosted_api_second_render_process_recovery(
      $1,$2,$3,$4,$5,$6,$7)`,
    [account, workspace, user, project, thirdAttempt, id(12), thirdBundle]), /identity drift/u);
    assert.equal((await db.query(`SELECT count(*)::int AS count
      FROM hosted_api_second_render_process_recoveries`)).rows[0].count, 1);
  } finally {
    await db.close();
  }
});

test("0208 is the exact manifest tail", () => {
  ...
  assert.deepEqual([tail.version, tail.name, tail.filename],
    [208, "hosted_api_second_render_process_recovery", "0208_hosted_api_second_render_process_recovery.sql"]);
  assert.equal(tail.sha256, `sha256:${createHash("sha256").update(migration208).digest("hex")}`);
});
