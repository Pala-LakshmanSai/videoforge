...
async function insertContextProfile(executor, { id, operation = "voiceover-context-v6" }) {
  const configuration = {
  ...
  await executor.query(
    `INSERT INTO execution_profiles (
       ...
       $1,$2,$3,'Hosted Runware voiceover context',4,'PROMPT','TESTED','RUNWARE',
...
test("0065 reuses the workspace no-reasoning GPT-5 Nano profile for a new project claim", async () => {
  await withPgcryptoMigratedDatabase(async ({ executor }) => {
    ...
    const profiles = await executor.query(
      `SELECT id::text AS id FROM execution_profiles
        WHERE workspace_id=$1 AND name='Hosted Runware voiceover context' AND revision=4`,
      [IDS.workspaceA],
    );
...
test("0065 fails before claim or reservation when the no-reasoning profile drifted", async () => {
  await withPgcryptoMigratedDatabase(async ({ executor }) => {
    await seedLockedProjects(executor);
    await executor.query(`SELECT set_config($1, $2, false)`, [
      TENANT_PRINCIPAL_SETTING,
