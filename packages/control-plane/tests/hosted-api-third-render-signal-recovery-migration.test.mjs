...
const thirdAttempt = id(10), fourthAttempt = id(12);
...
test("0209 permits one exact provider-free signal recovery on worker 0.1.36+", async () => {
  const db = new PGlite();
  try {
    await db.exec(`
...
END; $$;
    ...
    assert.equal((await db.query(`SELECT has_function_privilege(
      'videoforge_v209_runtime_dc9612d6',
      'public.videoforge_prepare_hosted_api_third_render_signal_recovery(uuid,uuid,uuid,uuid,uuid,uuid,text)',
    ...
    const call = (bundle, version = "0.1.36", failedId = thirdAttempt, retryId = fourthAttempt) => db.query(
    ...
    await db.query(`UPDATE video_runtime_states SET stage='FAILED',terminal_reason='RENDER_FAILURE',
      terminal_at=now() WHERE id=$1`, [runtime]);
    await assert.rejects(call(fourthBundle, "0.1.35"), /requires worker 0.1.36/u);
    ...
    const state = (await db.query(`SELECT request.state AS request_state,
      ...
      JOIN hosted_api_third_render_signal_recoveries recovery
    ...
    assert.equal((await db.query(`SELECT count(*)::int AS count FROM hosted_cpu_job_attempts
      WHERE kind='RENDER'`)).rows[0].count, 3);
    assert.equal((await db.query(`SELECT detail->>'provider_actions_created' AS paid
      FROM video_runtime_events WHERE reason='LOCAL_RENDER_SIGNAL_RECOVERY'`)).rows[0].paid, "false");
    const replay = (await call(fourthBundle, "0.1.36", thirdAttempt, id(13))).rows[0].value;
    assert.deepEqual([replay.retry_attempt_id, replay.replayed], [fourthAttempt, true]);
    await assert.rejects(call(firstBundle, "0.1.36", thirdAttempt, id(13)), /identity drift/u);
    await assert.rejects(call(fourthBundle, "0.1.36", fourthAttempt, id(13)), /recovery/u);
    assert.equal((await db.query(`SELECT count(*)::int AS count
      FROM hosted_api_third_render_signal_recoveries`)).rows[0].count, 1);
  } finally {
    await db.close();
  }
});

test("0209 is the exact manifest tail", () => {
  ...
  assert.deepEqual([tail.version, tail.name, tail.filename],
    [209, "hosted_api_third_render_signal_recovery", "0209_hosted_api_third_render_signal_recovery.sql"]);
  assert.equal(tail.sha256, `sha256:${createHash("sha256").update(migration209).digest("hex")}`);
});
