-- Restore the execution-profile and reservation-sequence resolution.
--
-- 0157 removed the whole block between the existing-run guard and the task insert to drop a rebind
-- that ran too early, which also deleted the code that resolves the PROMPT execution profile and
-- allocates the cost reservation sequence. The attempt insert then failed on
-- `null value in column "execution_profile_id" of relation "attempts" violates not-null constraint`.
-- This revision puts that code back in its original position and keeps only the end-of-function
-- rebind from 0157, so the ordering is: guard -> profile -> sequence -> attempt rows -> run row.
