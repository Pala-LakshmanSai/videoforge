-- Stage 3 completion must conserve its fixed reservation.  The original completion
-- binding recorded the actual reported/settled cost but omitted the unused remainder.
-- Keep the acceptance transaction and semantic validation unchanged while appending the
-- positive remainder as one immutable RELEASED event.
...
BEGIN
  ...
  IF target.reserved_cost_micro_usd > supplied_reported_cost THEN
    ...
    VALUES(gen_random_uuid(),target.account_id,target.workspace_id,'PROJECT_REVISION',
      ...
      'hosted-voiceover-context:'||target.project_revision_id||':released',
