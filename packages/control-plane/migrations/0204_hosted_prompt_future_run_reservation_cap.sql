...
-- New runs reserve USD 0.25 per planned batch, capped at USD 8. Existing runs retain their
-- exact saved reservation and profile on eligible redispatch. This migration does not touch rows.
...
ALTER TABLE public.hosted_prompt_runs
  ADD CONSTRAINT hosted_prompt_runs_reserved_cost_micro_usd_check CHECK (
    ...
    (reserved_cost_micro_usd BETWEEN 250000 AND 8000000
...
BEGIN
  ...
  IF (existing.id IS NULL AND requested_reserved IS DISTINCT FROM
       least(8000000::numeric,planned_batch_count::numeric*250000::numeric))
     OR (existing.id IS NOT NULL AND redispatch AND
       requested_reserved IS DISTINCT FROM existing.reserved_cost_micro_usd) THEN
  ...
  END IF;
  -- Preserve the profile and reservation of every existing run, including old USD 2 runs.
  IF existing.id IS NOT NULL AND redispatch THEN
  ...
  VALUES(requested_profile_id,account_id,workspace_id,'Hosted Runware scene prompts',profile_revision,'PROMPT',
    'TESTED','RUNWARE',profile_config,profile_config_hash,profile_rate,now_at,now_at)
  ...
  SELECT * INTO existing_profile FROM public.execution_profiles profile
   WHERE profile.account_id=account_id AND profile.workspace_id=workspace_id
     AND profile.name='Hosted Runware scene prompts' AND profile.revision=profile_revision FOR SHARE;
  IF existing_profile.id IS NULL OR existing_profile.lane<>'PROMPT'
     ...
     OR existing_profile.maximum_rate_micro_usd<>profile_rate THEN
    RAISE EXCEPTION 'hosted prompt execution profile drifted' USING ERRCODE='23514';
  END IF;
