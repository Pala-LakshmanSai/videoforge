-- Bounded redispatch for a hosted voiceover-context request whose provider confirmed it produced
-- no usable result.
--
-- Before this migration a transient Runware 5xx (or a provider-confirmed upstream failure) left the
-- revision's context row in state FAILED/UNKNOWN, and every later `POST /context` returned
-- 409 HOSTED_CONTEXT_ALREADY_CLAIMED while `POST /reconcile-context` answered "cannot resume this
-- task". The project was stranded at stage 3 with no path forward inside the product.
--
...
BEGIN
  ...
  END IF;
  next_ordinal:=existing.redispatch_count+2;
