-- Opens the stage-3 allowance once more, to twenty-two.
--
-- Every attempt the previous allowance paid for was spent against a provider-side or product-side
-- cause that is now fixed (a dead model, a token ceiling too small for the first replacement's own
-- reasoning, then its cost above this stage's reservation). The reader that is pinned now costs
-- seventy-three micro-USD per extraction and answers in about a second, so the remaining question is
-- whether the acceptance path itself holds - and a revision whose budget is spent cannot be observed.
-- The bound stays a small constant and every attempt is separately reserved.
...
BEGIN
  ...
  END IF;
  IF existing.redispatch_count >= 22 THEN
