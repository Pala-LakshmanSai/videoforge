-- Make a redispatched prompt attempt own unique durable keys.
--
-- `videoforge_prepare_hosted_prompt_run` keyed three rows on the revision alone:
--   generation_tasks (workspace_id, owner_type, owner_id=revision_id, task_key='prompt:scene-batch:1')
--   attempts        (workspace_id, idempotency_key='hosted-prompt:<revision>')
--   outbox          (workspace_id, dedupe_key='hosted-prompt:<revision>')
-- so the second attempt for a revision died with SQLSTATE 23505 before it could reach the provider --
-- observed live as `hosted_prompt_unexpected_failure ... duplicate key value violates unique
-- constraint "generation_tasks_workspace_id_owner_type_owner_id_task_key_key"`, surfacing to the
-- operator as HOSTED_PROMPT_EXECUTION_UNKNOWN and leaving stage 5 unrecoverable.
--
-- A redispatch now appends its own attempt id to those keys, so each attempt owns its rows and the
-- failed attempt stays on record. A first dispatch keeps the original keys exactly, and every
-- reader already matches `prompt:scene-batch:%`, so nothing else observes the difference.
