-- One final provider-free render after the consumed process recovery fails locally.
-- Preserve the exact Kie/Fal receipts, render plan, and input identities.
CREATE TABLE public.hosted_api_third_render_signal_recoveries (
  generation_request_id uuid PRIMARY KEY
    REFERENCES public.hosted_api_second_render_process_recoveries(generation_request_id),
...
ALTER TABLE public.hosted_api_third_render_signal_recoveries ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.hosted_api_third_render_signal_recoveries FORCE ROW LEVEL SECURITY;
CREATE POLICY hosted_api_third_render_signal_recoveries_tenant
  ON public.hosted_api_third_render_signal_recoveries
  USING (account_id=public.videoforge_current_account_id())
  WITH CHECK (account_id=public.videoforge_current_account_id());
REVOKE ALL ON public.hosted_api_third_render_signal_recoveries FROM PUBLIC;
...
BEGIN
  ...
  IF strpos(definition,$old$  IF OLD.stage IN ('COMPLETE', 'FAILED', 'CANCELED') THEN$old$)=0
     OR strpos(definition,'hosted_api_second_render_process_recoveries')=0 THEN
    RAISE EXCEPTION 'third render signal recovery runtime trigger preimage drifted';
  END IF;
  definition:=replace(definition,
    ...
    $new$  IF OLD.stage='FAILED' AND OLD.terminal_reason='RENDER_FAILURE'
     ...
     AND EXISTS (
       SELECT 1 FROM public.hosted_api_third_render_signal_recoveries recovery
...
CREATE FUNCTION public.videoforge_prepare_hosted_api_third_render_signal_recovery(
...
BEGIN
  IF public.videoforge_current_account_id() IS DISTINCT FROM supplied_account_id
    ...
    RAISE EXCEPTION 'third render signal recovery tenant or identity invalid' USING ERRCODE='42501';
  END IF;
  ...
  IF request.id IS NULL THEN
    RAISE EXCEPTION 'third render signal recovery request missing' USING ERRCODE='23514';
  END IF;
  ...
  IF third_recovery.generation_request_id IS NOT NULL THEN
    IF third_recovery.failed_attempt_id<>supplied_failed_attempt_id
       OR third_recovery.replacement_bundle_sha256<>supplied_replacement_bundle_sha256
    ...
    RETURN jsonb_build_object('schema_version','videoforge-hosted-render-disk-recovery/v1',
      ...
      'recovery_kind','SIGNAL','replayed',true);
  END IF;
  ...
  IF first_recovery.generation_request_id IS NULL OR first_recovery.state<>'CONSUMED'
     ...
     OR second_recovery.replacement_bundle_sha256 IS DISTINCT FROM failed.image_digest
     ...
     OR EXISTS (SELECT 1 FROM public.hosted_cpu_upload_authorities authority
       WHERE authority.account_id=request.account_id AND authority.workspace_id=request.workspace_id
         AND authority.attempt_id IN (failed.id,second.id,original.id) AND authority.issued_at IS NOT NULL)
    ...
    RAISE EXCEPTION 'third render signal recovery evidence rejected' USING ERRCODE='23514';
  END IF;
  INSERT INTO public.hosted_api_third_render_signal_recoveries(
  ...
  UPDATE public.hosted_api_third_render_signal_recoveries SET state='CONSUMED'
  ...
  VALUES(md5('hosted-api-third-render-signal-recovery:'||request.id::text)::uuid,
    request.account_id,request.workspace_id,runtime.id,request.project_revision_id,NULL,
    'FAILED','RENDERING','LOCAL_RENDER_SIGNAL_RECOVERY',
  ...
  RETURN jsonb_build_object('schema_version','videoforge-hosted-render-disk-recovery/v1',
    ...
    'recovery_kind','SIGNAL','replayed',false);
END;
$$;
REVOKE ALL ON FUNCTION public.videoforge_prepare_hosted_api_third_render_signal_recovery(
  uuid,uuid,uuid,uuid,uuid,uuid,text) FROM PUBLIC;

-- The existing seven-argument route keeps all earlier recoveries; only the consumed
-- 0208 attempt is eligible for this additional process-signal recovery.
