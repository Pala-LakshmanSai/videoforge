-- 0177_hosted_prompt_redispatch_rebind.sql
--
-- Restores the rebind branch the live prompt prepare capability lost. An approved redispatch used to
-- insert a second hosted_prompt_runs row, which the unique constraint on
-- (account_id, workspace_id, project_revision_id) refuses: every stage-5 redispatch died with
-- sqlstate 23505 before reaching the provider, so a stranded run could never be replaced even by the
-- bounded redispatch that exists for exactly that case. This is 0156's rebind branch, re-declared on
-- top of the current definition so the gemini prompt profile and its revision stay untouched.
