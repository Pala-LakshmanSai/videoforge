-- One provider-free render retry after a definitive local disk preflight failure.
-- The accepted Kie/Fal jobs, receipts, request identity, and render plan are unchanged.
CREATE TABLE public.hosted_api_render_recoveries (
...
ALTER TABLE public.hosted_api_render_recoveries ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.hosted_api_render_recoveries FORCE ROW LEVEL SECURITY;
CREATE POLICY hosted_api_render_recoveries_tenant ON public.hosted_api_render_recoveries
  USING (account_id=public.videoforge_current_account_id())
  WITH CHECK (account_id=public.videoforge_current_account_id());
REVOKE ALL ON public.hosted_api_render_recoveries FROM PUBLIC;

-- The runtime normally rejects every terminal transition. Permit this one transition only while
-- the security-definer capability has inserted its private, transaction-local PREPARING proof.
DO $migration$
DECLARE definition text;
BEGIN
  SELECT pg_get_functiondef('public.videoforge_validate_video_runtime_state()'::regprocedure)
    INTO definition;
  IF strpos(definition,$old$  IF OLD.stage IN ('COMPLETE', 'FAILED', 'CANCELED') THEN$old$)=0 THEN
    RAISE EXCEPTION 'render recovery runtime trigger preimage drifted';
  END IF;
  definition:=replace(definition,
    ...
    $new$  IF OLD.stage='FAILED' AND OLD.terminal_reason='RENDER_FAILURE'
     ...
     AND EXISTS (
       SELECT 1 FROM public.hosted_api_render_recoveries recovery
...
CREATE FUNCTION public.videoforge_prepare_hosted_api_render_disk_recovery(
...
DECLARE
  ...
  recovery public.hosted_api_render_recoveries%ROWTYPE;
...
BEGIN
  IF public.videoforge_current_account_id() IS DISTINCT FROM supplied_account_id
    ...
    RAISE EXCEPTION 'render disk recovery tenant or identity invalid' USING ERRCODE='42501';
  END IF;
  ...
  IF request.id IS NULL THEN
    RAISE EXCEPTION 'render disk recovery request missing' USING ERRCODE='23514';
  END IF;
  SELECT row.* INTO recovery FROM public.hosted_api_render_recoveries row
    WHERE row.generation_request_id=request.id FOR UPDATE;
  IF recovery.generation_request_id IS NOT NULL THEN
    IF recovery.failed_attempt_id<>supplied_failed_attempt_id OR recovery.state<>'CONSUMED' THEN
      RAISE EXCEPTION 'render disk recovery identity drift' USING ERRCODE='23514';
    END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM public.projects project
    ...
    RAISE EXCEPTION 'render disk recovery evidence rejected' USING ERRCODE='23514';
  END IF;
  INSERT INTO public.hosted_api_render_recoveries(
  ...
  UPDATE public.hosted_api_render_recoveries SET state='CONSUMED'
  ...
  VALUES(md5('hosted-api-render-disk-recovery:'||request.id::text)::uuid,
    request.account_id,request.workspace_id,runtime.id,request.project_revision_id,NULL,
    'FAILED','RENDERING','LOCAL_RENDER_DISK_RECOVERY',
...
END;
$$;
REVOKE ALL ON FUNCTION public.videoforge_prepare_hosted_api_render_disk_recovery(
  uuid,uuid,uuid,uuid,uuid,uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.videoforge_prepare_hosted_api_render_disk_recovery(
