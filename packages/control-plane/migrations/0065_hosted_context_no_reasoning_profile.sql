-- Stage 3 v5 proved low latency but returned no complete valid structured object. Preserve
-- immutable revisions 1 through 3 and select revision 4 for request v6, which disables reasoning
-- while restoring sufficient bounded output space. Stage 5 remains on its existing DeepSeek profile.
...
DECLARE
  ...
  profile_config jsonb:='{"model":"openai-gpt-5-nano","operation":"voiceover-context-v6","provider":"runware"}'::jsonb;
...
BEGIN
  ...
  VALUES(profile_id,account_id,workspace_id,'Hosted Runware voiceover context',4,'PROMPT','TESTED',
  ...
  SELECT * INTO existing_profile FROM public.execution_profiles profile
   WHERE profile.account_id=account_id AND profile.workspace_id=workspace_id
     AND profile.name='Hosted Runware voiceover context' AND profile.revision=4 FOR SHARE;
