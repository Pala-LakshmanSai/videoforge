-- 0180_hosted_prompt_reservation_six_hundred_thousand.sql
--
-- Raises the prompt run's reservation and the prompt execution profile's rate from USD 0.04 to
-- USD 0.60, on a new profile revision (5), because that is what the work actually costs: one
-- 10-scene batch measured USD 0.12 at the pinned model and a revision plans three of them, so the
-- old reservation refused every batch that the provider had already answered. The capability's
-- authority gate, the profile gate, the fresh reservation cost event and the rebind path all move
-- together, so no caller can dispatch a run under the old bound.
