-- One further provider-free render attempt after the disk recovery's local I/O failure.
-- The first failed attempt remains the disk-space failure authorized by 0197. Neither
-- attempt may have issued an upload authority or produced a durable result.
CREATE TABLE public.hosted_api_render_io_recoveries (
  generation_request_id uuid PRIMARY KEY REFERENCES public.hosted_api_render_recoveries(generation_request_id),
...
ALTER TABLE public.hosted_api_render_io_recoveries ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.hosted_api_render_io_recoveries FORCE ROW LEVEL SECURITY;
CREATE POLICY hosted_api_render_io_recoveries_tenant ON public.hosted_api_render_io_recoveries
  USING (account_id=public.videoforge_current_account_id())
  WITH CHECK (account_id=public.videoforge_current_account_id());
REVOKE ALL ON public.hosted_api_render_io_recoveries FROM PUBLIC;

-- Retain 0197's terminal-state exception and add the same transaction-local proof
-- for the second, separately recorded attempt.
DO $migration$
...
BEGIN
  ...
  IF strpos(definition,$old$  IF OLD.stage IN ('COMPLETE', 'FAILED', 'CANCELED') THEN$old$)=0
     OR strpos(definition,'hosted_api_render_recoveries')=0 THEN
    RAISE EXCEPTION 'render I/O recovery runtime trigger preimage drifted';
  END IF;
  definition:=replace(definition,
    ...
    $new$  IF OLD.stage='FAILED' AND OLD.terminal_reason='RENDER_FAILURE'
     ...
     AND EXISTS (
       SELECT 1 FROM public.hosted_api_render_io_recoveries recovery
...
CREATE FUNCTION public.videoforge_prepare_hosted_api_render_io_recovery(
...
BEGIN
  IF public.videoforge_current_account_id() IS DISTINCT FROM supplied_account_id
    ...
    RAISE EXCEPTION 'render I/O recovery tenant or identity invalid' USING ERRCODE='42501';
  END IF;
  ...
  IF request.id IS NULL THEN
    RAISE EXCEPTION 'render I/O recovery request missing' USING ERRCODE='23514';
  END IF;
  ...
  IF second_recovery.generation_request_id IS NOT NULL THEN
    IF second_recovery.failed_attempt_id<>supplied_failed_attempt_id
      ...
      RAISE EXCEPTION 'render I/O recovery identity drift' USING ERRCODE='23514';
    END IF;
    RETURN jsonb_build_object('schema_version','videoforge-hosted-render-disk-recovery/v1',
      ...
      'recovery_kind','IO','replayed',true);
  END IF;
  ...
  IF first_recovery.generation_request_id IS NULL OR first_recovery.state<>'CONSUMED'
    ...
    RAISE EXCEPTION 'render I/O recovery evidence rejected' USING ERRCODE='23514';
  END IF;
  INSERT INTO public.hosted_api_render_io_recoveries(
  ...
  UPDATE public.hosted_api_render_io_recoveries SET state='CONSUMED'
  ...
  VALUES(md5('hosted-api-render-io-recovery:'||request.id::text)::uuid,
    request.account_id,request.workspace_id,runtime.id,request.project_revision_id,NULL,
    'FAILED','RENDERING','LOCAL_RENDER_IO_RECOVERY',
  ...
  RETURN jsonb_build_object('schema_version','videoforge-hosted-render-disk-recovery/v1',
    ...
    'recovery_kind','IO','replayed',false);
END;
$$;
REVOKE ALL ON FUNCTION public.videoforge_prepare_hosted_api_render_io_recovery(
  uuid,uuid,uuid,uuid,uuid,uuid) FROM PUBLIC;

-- The route has one entry point; 0197's original one-attempt semantics remain intact.
...
SET search_path=public,pg_catalog AS $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.hosted_api_render_recoveries recovery
    ...
    RETURN public.videoforge_prepare_hosted_api_render_io_recovery(
  ...
  END IF;
  RETURN public.videoforge_prepare_hosted_api_render_disk_recovery(
    supplied_account_id,supplied_workspace_id,supplied_user_id,
    supplied_project_id,supplied_failed_attempt_id,supplied_retry_attempt_id)
    || jsonb_build_object('recovery_kind','DISK');
END;
$$;
...
GRANT EXECUTE ON FUNCTION public.videoforge_prepare_hosted_api_render_recovery(
  uuid,uuid,uuid,uuid,uuid,uuid) TO videoforge_v209_runtime_dc9612d6;
