-- 0194_hosted_prompt_dynamic_reservation_and_batch_claims.sql
--
-- Future-only hardening for adaptive prompt dispatch. The current UNKNOWN run remains terminal
-- until an independent provider reconciliation proves a recoverable task; this migration does not
-- revive or mutate it. New runs receive the exact batch-count reservation and every provider POST
-- must have an immutable, request-hash-bound claim first.

ALTER TABLE public.hosted_prompt_runs
  DROP CONSTRAINT IF EXISTS hosted_prompt_runs_reserved_cost_micro_usd_check;
ALTER TABLE public.hosted_prompt_runs
  ADD CONSTRAINT hosted_prompt_runs_reserved_cost_micro_usd_check CHECK (
    reserved_cost_micro_usd IN (40000,600000) OR
    (reserved_cost_micro_usd BETWEEN 250000 AND 2000000
      AND mod(reserved_cost_micro_usd,250000)=0)
  );
...
DECLARE
  ...
  profile_config jsonb:='{"model":"google:gemini@3.5-flash","operation":"scene-prompt-writer-v2","provider":"runware"}'::jsonb;
...
BEGIN
  IF public.videoforge_current_account_id() IS DISTINCT FROM account_id
     ...
     OR supplied->>'reserved_cost_micro_usd' IS NULL
     OR supplied->>'reserved_cost_micro_usd' !~ '^[0-9]+$'
  ...
  requested_reserved:=(supplied->>'reserved_cost_micro_usd')::numeric;
  IF requested_reserved IS DISTINCT FROM
       least(2000000::numeric,planned_batch_count::numeric*250000::numeric) THEN
    RAISE EXCEPTION 'hosted prompt reservation is invalid' USING ERRCODE='23514';
  END IF;
  ...
  VALUES(requested_profile_id,account_id,workspace_id,'Hosted Runware scene prompts',6,'PROMPT',
    'TESTED','RUNWARE',profile_config,profile_config_hash,2000000,now_at,now_at)
  ...
  SELECT * INTO existing_profile FROM public.execution_profiles profile
   WHERE profile.account_id=account_id AND profile.workspace_id=workspace_id
     AND profile.name='Hosted Runware scene prompts' AND profile.revision=6 FOR SHARE;
  IF existing_profile.id IS NULL OR existing_profile.lane<>'PROMPT'
     ...
     OR existing_profile.maximum_rate_micro_usd<>2000000 THEN
  ...
  -- A redispatch supersedes the previous attempt's scene-batch task. The old row stays as
  -- history, but it must stop reading as required: the dispatch admission refuses any revision
  -- whose required scene-batch task is not COMPLETE, so without this demotion one dispatch
  -- timeout poisons the revision for every later generation attempt.
  IF redispatch THEN
    UPDATE public.generation_tasks task
       SET required=false, updated_at=now_at
     WHERE task.account_id=account_id AND task.workspace_id=workspace_id
       AND task.project_revision_id=revision_id AND task.lane='PROMPT'
       AND task.task_key LIKE 'prompt:scene-batch:%'
       AND task.state<>'COMPLETE' AND task.required;
  END IF;
  ...
  VALUES(attempt_id,account_id,workspace_id,task_id,1,'hosted-prompt:'||revision_id||CASE WHEN redispatch THEN ':'||attempt_id::text ELSE '' END,
    ...
    jsonb_build_object('provider','runware','model','google:gemini@3.5-flash',
  ...
  VALUES(cost_id,account_id,workspace_id,'PROJECT_REVISION',revision_id,task_id,attempt_id,
    reservation_sequence,'RESERVED',requested_reserved,'hosted-prompt:'||revision_id||':reserved'||CASE WHEN redispatch THEN ':'||attempt_id::text ELSE '' END,
    jsonb_build_object('provider','runware','model','google:gemini@3.5-flash',
  ...
  -- so an approved redispatch rebinds that row to the fresh attempt instead of inserting a second one.
  -- Without this branch every redispatch died on that unique constraint before it reached the provider,
  -- which is what kept a stranded stage-5 run stranded no matter how often a caller asked for it. The
  -- check constraints require the reset in the same statement: DISPATCHING implies finished_at IS NULL,
  -- no problem_code, provider_may_have_charged = false and no acceptance fingerprint -- all of which
  -- describe this attempt truthfully, because the gate only approves revisions whose previous attempt
  -- produced no durable accepted prompt set. redispatch_count bounds how often this may happen.
  IF existing.id IS NOT NULL THEN
    UPDATE public.hosted_prompt_runs run
       SET task_id=task_id, attempt_id=attempt_id, outbox_id=outbox_id,
           ...
           reserved_cost_micro_usd=requested_reserved, reservation_cost_sequence=reservation_sequence,
    ...
    RETURN jsonb_build_object('created',true,'state','DISPATCHING','redispatch',true,'run_id',existing.id,
  ...
  VALUES(run_id,account_id,workspace_id,project_id,revision_id,timeline_id,task_id,attempt_id,
    outbox_id,profile_id,'DISPATCHING',input_hash,claim_hash,requested_reserved,reservation_sequence,
...
CREATE TABLE public.hosted_prompt_batch_claims (
  ...
  provider_task_uuid text NOT NULL CHECK (length(provider_task_uuid) BETWEEN 1 AND 240),
...
CREATE INDEX hosted_prompt_batch_claims_run_idx
  ON public.hosted_prompt_batch_claims(account_id, workspace_id, run_id, batch_ordinal);
CREATE TRIGGER hosted_prompt_batch_claims_tenant_write_guard
  BEFORE INSERT OR UPDATE ON public.hosted_prompt_batch_claims
  FOR EACH ROW EXECUTE FUNCTION public.videoforge_assert_tenant_write();
CREATE TRIGGER hosted_prompt_batch_claims_append_only
  BEFORE UPDATE OR DELETE ON public.hosted_prompt_batch_claims
  FOR EACH ROW EXECUTE FUNCTION public.videoforge_reject_immutable_row();
ALTER TABLE public.hosted_prompt_batch_claims ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.hosted_prompt_batch_claims FORCE ROW LEVEL SECURITY;
CREATE POLICY hosted_prompt_batch_claims_tenant_rls ON public.hosted_prompt_batch_claims
...
ALTER TABLE public.hosted_prompt_batch_progress
  ...
  ADD CONSTRAINT hosted_prompt_batch_progress_claim_fk
    FOREIGN KEY (account_id, workspace_id, claim_id)
    REFERENCES public.hosted_prompt_batch_claims(account_id, workspace_id, id)
    ON DELETE RESTRICT;
CREATE INDEX hosted_prompt_batch_progress_claim_idx
  ON public.hosted_prompt_batch_progress(account_id, workspace_id, claim_id);

CREATE FUNCTION public.videoforge_claim_hosted_prompt_batch(
  supplied_run_id uuid,
  supplied_batch_ordinal integer,
  supplied_provider_task_uuid text,
  supplied_request_bytes text,
...
DECLARE
  ...
  existing public.hosted_prompt_batch_claims%ROWTYPE;
  prior_batch_count integer;
  expected_hash text;
BEGIN
  SELECT run_row.* INTO run
    FROM public.hosted_prompt_runs run_row
   WHERE run_row.id=supplied_run_id
   FOR UPDATE;
  expected_hash:='sha256:'||encode(digest(convert_to(coalesce(supplied_request_bytes,''),'UTF8'),'sha256'),'hex');
  IF run.id IS NULL
     OR public.videoforge_current_account_id() IS DISTINCT FROM run.account_id
     OR run.state<>'DISPATCHING'
     OR run.planned_batch_count IS NULL
     OR supplied_batch_ordinal IS NULL
     OR supplied_batch_ordinal<0
     OR supplied_batch_ordinal>=run.planned_batch_count
     OR supplied_provider_task_uuid IS NULL
     OR length(supplied_provider_task_uuid) NOT BETWEEN 1 AND 240
     OR supplied_request_bytes IS NULL
     OR octet_length(supplied_request_bytes) NOT BETWEEN 1 AND 8388608
     OR supplied_request_hash IS DISTINCT FROM expected_hash
     OR supplied_request_hash !~ '^sha256:[0-9a-f]{64}$' THEN
    RAISE EXCEPTION 'hosted prompt batch claim is invalid' USING ERRCODE='23514';
  END IF;
  ...
  IF supplied_batch_ordinal<>prior_batch_count THEN
    RAISE EXCEPTION 'hosted prompt batch claim is out of order' USING ERRCODE='23514';
  END IF;
  SELECT claim_row.* INTO existing
    FROM public.hosted_prompt_batch_claims claim_row
   WHERE claim_row.account_id=run.account_id AND claim_row.workspace_id=run.workspace_id
     AND claim_row.run_id=run.id AND claim_row.batch_ordinal=supplied_batch_ordinal
   FOR UPDATE;
  IF existing.id IS NOT NULL THEN
    IF existing.task_id=run.task_id AND existing.attempt_id=run.attempt_id
       AND existing.outbox_id=run.outbox_id
       AND existing.provider_task_uuid=supplied_provider_task_uuid
       AND existing.request_bytes=supplied_request_bytes
       AND existing.request_hash=supplied_request_hash THEN
      RETURN false;
    END IF;
    RAISE EXCEPTION 'hosted prompt batch claim identity drifted' USING ERRCODE='23514';
  END IF;
  INSERT INTO public.hosted_prompt_batch_claims(
    ...
    provider_task_uuid,request_bytes,request_hash
  ) VALUES (
    gen_random_uuid(),run.account_id,run.workspace_id,run.id,run.task_id,run.attempt_id,
    run.outbox_id,supplied_batch_ordinal,supplied_provider_task_uuid,
    supplied_request_bytes,supplied_request_hash
  );
  RETURN true;
END;
$$;
...
BEGIN
  ...
  SELECT count(*)::integer, coalesce(sum(progress.scene_count),0)::integer,
   ...
   WHERE progress.account_id=run.account_id AND progress.workspace_id=run.workspace_id
     AND progress.run_id=run.id;
  SELECT claim_row.id INTO claim_id
    FROM public.hosted_prompt_batch_claims claim_row
   WHERE claim_row.account_id=run.account_id AND claim_row.workspace_id=run.workspace_id
     AND claim_row.run_id=run.id AND claim_row.task_id=run.task_id
     AND claim_row.attempt_id=run.attempt_id AND claim_row.outbox_id=run.outbox_id
     AND claim_row.batch_ordinal=batch_ordinal AND claim_row.request_hash=request_hash;

  IF run.id IS NULL OR public.videoforge_current_account_id() IS DISTINCT FROM run.account_id
     OR run.state<>'DISPATCHING'
     OR (claim_id IS NULL AND run.reserved_cost_micro_usd NOT IN (40000,600000))
  ...
  INSERT INTO public.hosted_prompt_batch_progress(
    id,account_id,workspace_id,run_id,claim_id,batch_ordinal,first_scene_ordinal,scene_count,
    ...
    batch_id,run.account_id,run.workspace_id,run.id,claim_id,batch_ordinal,first_scene_ordinal,
...
END;
$$;
REVOKE ALL ON TABLE public.hosted_prompt_batch_claims FROM PUBLIC;
REVOKE ALL ON FUNCTION public.videoforge_claim_hosted_prompt_batch(uuid,integer,text,text,text) FROM PUBLIC;
