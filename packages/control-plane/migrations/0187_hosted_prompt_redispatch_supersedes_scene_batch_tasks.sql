-- A redispatch must supersede the prior attempt's scene-batch task, not sit next to it.
-- 0177 rebinds the run row, but the superseded generation_task stayed required=true/FAILED,
-- so videoforge_assert_hosted_v209_* admission raised 'hosted V2-09 prompts are not ready'
-- (55000) on every later dispatch of that revision. The owner function keeps its live body
-- (0168 as amended by 0176/0180); this migration only adds the demotion before the insert.
...
BEGIN
  IF public.videoforge_current_account_id() IS DISTINCT FROM account_id
     ...
     OR (supplied->>'reserved_cost_micro_usd')::bigint<>600000
  ...
  VALUES(requested_profile_id,account_id,workspace_id,'Hosted Runware scene prompts',5,'PROMPT',
    'TESTED','RUNWARE',profile_config,profile_config_hash,600000,now_at,now_at)
  ...
  SELECT * INTO existing_profile FROM public.execution_profiles profile
   WHERE profile.account_id=account_id AND profile.workspace_id=workspace_id
     AND profile.name='Hosted Runware scene prompts' AND profile.revision=5 FOR SHARE;
  IF existing_profile.id IS NULL OR existing_profile.lane<>'PROMPT'
     ...
     OR existing_profile.maximum_rate_micro_usd<>600000 THEN
  ...
  VALUES(cost_id,account_id,workspace_id,'PROJECT_REVISION',revision_id,task_id,attempt_id,
    reservation_sequence,'RESERVED',600000,'hosted-prompt:'||revision_id||':reserved'||CASE WHEN redispatch THEN ':'||attempt_id::text ELSE '' END,
    jsonb_build_object('provider','runware','model','google:gemini@3.5-flash',
  ...
  IF existing.id IS NOT NULL THEN
    UPDATE public.hosted_prompt_runs run
       SET task_id=task_id, attempt_id=attempt_id, outbox_id=outbox_id,
           ...
           reserved_cost_micro_usd=600000, reservation_cost_sequence=reservation_sequence,
  ...
  VALUES(run_id,account_id,workspace_id,project_id,revision_id,timeline_id,task_id,attempt_id,
    outbox_id,profile_id,'DISPATCHING',input_hash,claim_hash,600000,reservation_sequence,
