-- Rebind the single prompt run row when an approved redispatch replaces a failed attempt.
--
-- The schema enforces one run row per revision (`hosted_prompt_runs_account_id_workspace_id_project_revision_key`),
-- so creating a second row is impossible by design; the previous revision of this function tried and
-- died on that constraint after 0154/0155 removed the task/attempt/outbox/cost collisions.
--
-- An approved redispatch now rebinds the existing row to the fresh attempt (new task/attempt/outbox
-- and profile ids, a new reservation sequence) and clears the terminal fields the check constraints
-- tie to DISPATCHING. `redispatch_count` is added here and incremented, mirroring the voiceover
-- context budget, because the revision still owns exactly one run row and the row count can no
-- longer express how many attempts were spent. A duplicate POST without approval still returns
-- created=false and no state is rewritten.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'existing_run_count',(SELECT count(*) FROM public.hosted_prompt_runs run
      WHERE run.account_id=supplied_account_id AND run.workspace_id=supplied_workspace_id
        AND run.project_revision_id=selected.revision_id),
    'existing_run_redispatch_count',(SELECT run.redispatch_count FROM public.hosted_prompt_runs run
      WHERE run.account_id=supplied_account_id AND run.workspace_id=supplied_workspace_id
        AND run.project_revision_id=selected.revision_id
      ORDER BY run.created_at DESC LIMIT 1),
...
BEGIN
  ...
  -- The check constraints require the reset in the same statement: DISPATCHING implies finished_at IS
  -- NULL, no problem_code, provider_may_have_charged = false, and no acceptance fingerprint -- all of
  -- which describe this attempt truthfully because the gate only approves revisions whose previous
  -- attempt produced no durable accepted prompt set. redispatch_count bounds how often this may happen.
