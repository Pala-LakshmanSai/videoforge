-- Makes a rejected context attempt retryable inside the same bound.
--
-- The gate exists so the product does not wait forever on a request the provider will not serve, but
-- the rejection that stranded this revision was the product's own request shape: the pinned model
-- bills its reasoning against the token ceiling, the old ceiling truncated the answer, and the
-- transport refused the truncated document. That cause is fixed in the application, and a stored
-- rejection must not keep a revision stopped once the request itself has changed - the redispatch
-- budget, not the problem-code gate, is what bounds the attempts. Every other gate is unchanged.
