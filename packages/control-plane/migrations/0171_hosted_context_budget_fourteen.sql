-- Raises the stage-3 redispatch budget to fourteen, matching the application constant.
--
-- The allowance was ten when the pinned text model was replaced and again when the replacement's own
-- token ceiling was found to truncate its answers; a revision that spent every attempt on a
-- provider-side cause (a dead model, then a truncated response the transport rightly refuses) must be
-- able to continue once the cause is gone. The bound stays a small constant and every redispatch is
-- still separately reserved, so this remains a bound on wasted attempts rather than on money. Nothing
-- else about the capability changes: the profile revision, the retryable problem codes and the
-- no-accepted-result gate are exactly as 0170 left them.
...
BEGIN
  ...
  END IF;
  IF existing.redispatch_count >= 14 THEN
