-- Restores the prompt-writer capability's declared model to the one the product actually sends.
--
-- Migration 0173 moved the context reader to google:gemma@4-31b, and its replacement of the model
-- literal also caught the prompt-writer function that trailed the same body, so the capability began
-- demanding a gemma profile while the profile row it reads - and the writer in
-- packages/pipeline/src/prompts/runware-deepseek-writer.ts - both name google:gemini@3.5-flash. Every
-- prompt dispatch then failed with 'hosted prompt execution profile drifted' (sqlstate 23514), which
-- the sweep recorded on every tick. This re-declares the prompt capability exactly as 0170 wrote it,
-- so all three agree again; nothing else changes.
