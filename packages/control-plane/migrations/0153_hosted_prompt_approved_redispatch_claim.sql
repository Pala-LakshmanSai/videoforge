-- Let an approved provider-failure redispatch claim a fresh prompt run.
--
-- `videoforge_prepare_hosted_prompt_run` refuses to create a run once the revision owns one, which
-- is right for a duplicate POST but wrong after a provider failure that produced no accepted prompt
-- set: the caller has nothing to resume, so the revision stranded at stage 5 forever. The route now
-- gates that case with `hostedPromptRedispatchable` (provider/transport problem code, no durable
-- accepted set, attempts left) and passes `redispatch: true` only then.
--
-- Nothing else changes: the new attempt gets its own reservation/task/attempt/outbox trio, the
-- previous run row is left untouched as the record of what the provider did, and the attempt budget
-- counts rows, so it bounds the number of redispatches the same way it did before.
