...
BEGIN
  ...
  VALUES(profile_id,account_id,workspace_id,'Hosted Runware voiceover context',6,'PROMPT','TESTED',
  ...
  SELECT * INTO existing_profile FROM public.execution_profiles profile
   WHERE profile.account_id=account_id AND profile.workspace_id=workspace_id
     AND profile.name='Hosted Runware voiceover context' AND profile.revision=6 FOR SHARE;
