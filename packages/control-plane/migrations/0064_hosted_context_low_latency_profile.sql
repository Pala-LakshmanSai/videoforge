-- Stage 3 now uses a lower-latency GPT-5 Nano request, while migration 0063 recorded the prior
-- context profile. Preserve immutable revisions 1 and 2 and select a new exact revision 3 profile
-- for every future context claim. Stage 5 remains on its existing DeepSeek scene-prompt profile.
...
DECLARE
  ...
  profile_config jsonb:='{"model":"openai-gpt-5-nano","operation":"voiceover-context-v5","provider":"runware"}'::jsonb;
...
BEGIN
  ...
  VALUES(profile_id,account_id,workspace_id,'Hosted Runware voiceover context',3,'PROMPT','TESTED',
  ...
  SELECT * INTO existing_profile FROM public.execution_profiles profile
   WHERE profile.account_id=account_id AND profile.workspace_id=workspace_id
     AND profile.name='Hosted Runware voiceover context' AND profile.revision=3 FOR SHARE;
  ...
  VALUES(attempt_id,account_id,workspace_id,task_id,1,'hosted-voiceover-context:'||revision_id,
    ...
    jsonb_build_object('provider','runware','model','openai-gpt-5-nano'),now_at,now_at,now_at);
  ...
  VALUES(cost_id,account_id,workspace_id,'PROJECT_REVISION',revision_id,task_id,attempt_id,
    ...
    jsonb_build_object('provider','runware','model','openai-gpt-5-nano'),now_at);
