-- Moves the hosted text execution profiles to a new revision so the pinned model can change.
--
-- The product expresses a text-model change as a new profile revision (4 -> 5 -> 6 -> 7 for the
-- voiceover-context reader, 1 -> 2 for the prompt writer), and the capabilities pin the revision they
-- read plus the exact configuration it must hold. Raising the model in place is caught as
-- 'hosted voiceover context execution profile drifted' - correctly, because a redispatch must not
-- silently switch the model under an existing lineage.
--
-- Migration 0167 pinned `google:gemini@3.5-flash` (the provider stopped serving `deepseek:v4@flash`)
-- but left both capabilities on revision 7, so every redispatch of an already-created profile was
-- refused. This migration creates the new revision rows for every workspace that already has the old
-- one and moves all three capabilities to them:
--   * Hosted Runware voiceover context: revision 8, {"model":"google:gemini@3.5-flash",
--     "operation":"voiceover-context-v10","provider":"runware"} (hash sha256:c95f9ef7af8c923270390d76993596c056cc497448fcd4d8d007d4dcd3dab6bc),
--   * Hosted Runware scene prompts: revision 3, {"model":"google:gemini@3.5-flash",
--     "operation":"scene-prompt-writer-v2","provider":"runware"} (hash sha256:428320e5c40e9f89065c9c93a63ae1ea6bef1120dcb486107be04dc293d7c16f).
--
-- Nothing else changes: the redispatch gates, budgets, problem-code lists and reservation amounts are
-- exactly as 0167 left them.
...
SELECT gen_random_uuid(),profile.account_id,profile.workspace_id,
       'Hosted Runware voiceover context',8,'PROMPT','TESTED','RUNWARE',
       '{"model":"google:gemini@3.5-flash","operation":"voiceover-context-v10","provider":"runware"}'::jsonb,'sha256:c95f9ef7af8c923270390d76993596c056cc497448fcd4d8d007d4dcd3dab6bc',10000,now(),now()
...
INSERT INTO public.execution_profiles(id,account_id,workspace_id,name,revision,lane,state,
  dispatch_target,configuration,configuration_hash,maximum_rate_micro_usd,checked_at,created_at)
SELECT gen_random_uuid(),profile.account_id,profile.workspace_id,
       'Hosted Runware scene prompts',3,'PROMPT','TESTED','RUNWARE',
       '{"model":"google:gemini@3.5-flash","operation":"scene-prompt-writer-v2","provider":"runware"}'::jsonb,'sha256:428320e5c40e9f89065c9c93a63ae1ea6bef1120dcb486107be04dc293d7c16f',40000,now(),now()
  FROM (SELECT DISTINCT account_id,workspace_id FROM public.execution_profiles
         WHERE name='Hosted Runware scene prompts' AND revision=2) profile
 ON CONFLICT ON CONSTRAINT execution_profiles_workspace_id_name_revision_key DO NOTHING;
...
DECLARE
  ...
  profile_config jsonb:='{"model":"google:gemini@3.5-flash","operation":"voiceover-context-v10","provider":"runware"}'::jsonb;
...
BEGIN
  ...
  INSERT INTO public.execution_profiles(id,account_id,workspace_id,name,revision,lane,state,
    dispatch_target,configuration,configuration_hash,maximum_rate_micro_usd,checked_at,created_at)
  VALUES(profile_id,account_id,workspace_id,'Hosted Runware voiceover context',8,'PROMPT','TESTED',
  ...
  SELECT * INTO existing_profile FROM public.execution_profiles profile
   WHERE profile.account_id=account_id AND profile.workspace_id=workspace_id
     AND profile.name='Hosted Runware voiceover context' AND profile.revision=8 FOR SHARE;
  ...
  VALUES(attempt_id,account_id,workspace_id,task_id,1,'hosted-voiceover-context:'||revision_id,
    ...
    jsonb_build_object('provider','runware','model','google:gemini@3.5-flash'),now_at,now_at,now_at);
  ...
  VALUES(cost_id,account_id,workspace_id,'PROJECT_REVISION',revision_id,task_id,attempt_id,
    ...
    jsonb_build_object('provider','runware','model','google:gemini@3.5-flash'),now_at);
...
BEGIN
  ...
  END IF;
  IF existing.redispatch_count >= 10 THEN
  ...
  VALUES(attempt_id,account_id,workspace_id,task_id,next_ordinal,
    ...
    'RUNNING','ACKNOWLEDGED','CLAIMED',profile_id,claim_hash,transcript_hash,'PENDING',
    jsonb_build_object('provider','runware','model','google:gemini@3.5-flash','redispatch',true),
  ...
  VALUES(cost_id,account_id,workspace_id,'PROJECT_REVISION',revision_id,task_id,attempt_id,
    ...
    jsonb_build_object('provider','runware','model','google:gemini@3.5-flash','redispatch',true),now_at);
...
BEGIN
  ...
  INSERT INTO public.execution_profiles(id,account_id,workspace_id,name,revision,lane,state,
    dispatch_target,configuration,configuration_hash,maximum_rate_micro_usd,checked_at,created_at)
  VALUES(requested_profile_id,account_id,workspace_id,'Hosted Runware scene prompts',3,'PROMPT',
    'TESTED','RUNWARE',profile_config,profile_config_hash,40000,now_at,now_at)
  ON CONFLICT ON CONSTRAINT execution_profiles_workspace_id_name_revision_key DO NOTHING;
  SELECT * INTO existing_profile FROM public.execution_profiles profile
   WHERE profile.account_id=account_id AND profile.workspace_id=workspace_id
     AND profile.name='Hosted Runware scene prompts' AND profile.revision=3 FOR SHARE;
  IF existing_profile.id IS NULL OR existing_profile.lane<>'PROMPT'
     OR existing_profile.state<>'TESTED' OR existing_profile.dispatch_target<>'RUNWARE'
     OR existing_profile.configuration IS DISTINCT FROM profile_config
     OR existing_profile.configuration_hash<>profile_config_hash
