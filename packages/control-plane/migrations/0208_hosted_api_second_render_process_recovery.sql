-- One further provider-free render after the exact first-input recovery reaches FFmpeg
-- and fails locally. No Kie/Fal job, accepted receipt, or render plan may change.
CREATE TABLE public.hosted_api_second_render_process_recoveries (
  generation_request_id uuid PRIMARY KEY
    REFERENCES public.hosted_api_first_render_input_recoveries(generation_request_id),
...
ALTER TABLE public.hosted_api_second_render_process_recoveries ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.hosted_api_second_render_process_recoveries FORCE ROW LEVEL SECURITY;
CREATE POLICY hosted_api_second_render_process_recoveries_tenant
  ON public.hosted_api_second_render_process_recoveries
  USING (account_id=public.videoforge_current_account_id())
  WITH CHECK (account_id=public.videoforge_current_account_id());
REVOKE ALL ON public.hosted_api_second_render_process_recoveries FROM PUBLIC;
...
BEGIN
  ...
  IF strpos(definition,$old$  IF OLD.stage IN ('COMPLETE', 'FAILED', 'CANCELED') THEN$old$)=0
     OR strpos(definition,'hosted_api_first_render_input_recoveries')=0 THEN
    RAISE EXCEPTION 'second render process recovery runtime trigger preimage drifted';
  END IF;
  definition:=replace(definition,
    ...
    $new$  IF OLD.stage='FAILED' AND OLD.terminal_reason='RENDER_FAILURE'
     ...
     AND EXISTS (
       SELECT 1 FROM public.hosted_api_second_render_process_recoveries recovery
...
CREATE FUNCTION public.videoforge_prepare_hosted_api_second_render_process_recovery(
...
BEGIN
  IF public.videoforge_current_account_id() IS DISTINCT FROM supplied_account_id
    ...
    RAISE EXCEPTION 'second render process recovery tenant or identity invalid' USING ERRCODE='42501';
  END IF;
  ...
  IF request.id IS NULL THEN
    RAISE EXCEPTION 'second render process recovery request missing' USING ERRCODE='23514';
  END IF;
  ...
  SELECT row.* INTO second_recovery FROM public.hosted_api_second_render_process_recoveries row
    WHERE row.generation_request_id=request.id FOR UPDATE;
  IF second_recovery.generation_request_id IS NOT NULL THEN
    IF second_recovery.failed_attempt_id<>supplied_failed_attempt_id
       OR second_recovery.replacement_bundle_sha256<>supplied_replacement_bundle_sha256
       OR second_recovery.state<>'CONSUMED' THEN
      RAISE EXCEPTION 'second render process recovery identity drift' USING ERRCODE='23514';
    END IF;
    RETURN jsonb_build_object('schema_version','videoforge-hosted-render-disk-recovery/v1',
      'revision_id',second_recovery.project_revision_id,
      'retry_attempt_id',second_recovery.retry_attempt_id,
      'recovery_kind','PROCESS','replayed',true);
  END IF;
  ...
  IF first_recovery.generation_request_id IS NULL OR first_recovery.state<>'CONSUMED'
     ...
     OR first_recovery.retry_attempt_id<>supplied_failed_attempt_id
     OR first_recovery.replacement_bundle_sha256 IS DISTINCT FROM failed.image_digest
     ...
     OR EXISTS (SELECT 1 FROM public.hosted_cpu_upload_authorities authority
       WHERE authority.account_id=request.account_id AND authority.workspace_id=request.workspace_id
         AND authority.attempt_id IN (failed.id,original.id) AND authority.issued_at IS NOT NULL)
     OR (SELECT count(*) FROM public.hosted_cpu_job_attempts job
       WHERE job.account_id=request.account_id AND job.workspace_id=request.workspace_id
         AND job.project_revision_id=request.project_revision_id AND job.kind='RENDER')<>2
     ...
     OR EXISTS (SELECT 1 FROM public.media_worker_leases lease
       WHERE lease.attempt_id IN (failed.id,original.id)
    ...
    RAISE EXCEPTION 'second render process recovery evidence rejected' USING ERRCODE='23514';
  END IF;
  INSERT INTO public.hosted_api_second_render_process_recoveries(
  ...
  UPDATE public.hosted_api_second_render_process_recoveries SET state='CONSUMED'
  ...
  VALUES(md5('hosted-api-second-render-process-recovery:'||request.id::text)::uuid,
    request.account_id,request.workspace_id,runtime.id,request.project_revision_id,NULL,
    'FAILED','RENDERING','LOCAL_RENDER_PROCESS_RECOVERY',
  ...
  RETURN jsonb_build_object('schema_version','videoforge-hosted-render-disk-recovery/v1',
    ...
    'recovery_kind','PROCESS','replayed',false);
END;
$$;
REVOKE ALL ON FUNCTION public.videoforge_prepare_hosted_api_second_render_process_recovery(
  uuid,uuid,uuid,uuid,uuid,uuid,text) FROM PUBLIC;

-- The existing route's seven-argument entry point retains all earlier paths.
...
SET search_path=public,pg_catalog AS $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.hosted_api_second_render_process_recoveries recovery
    ...
    RETURN public.videoforge_prepare_hosted_api_second_render_process_recovery(
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM public.hosted_api_first_render_input_recoveries first_recovery
       JOIN public.hosted_cpu_job_attempts failed
         ON failed.id=first_recovery.retry_attempt_id
       JOIN public.media_worker_leases lease ON lease.attempt_id=failed.id
       WHERE first_recovery.account_id=supplied_account_id
         AND first_recovery.workspace_id=supplied_workspace_id
         AND first_recovery.project_id=supplied_project_id
         AND first_recovery.retry_attempt_id=supplied_failed_attempt_id
         AND first_recovery.state='CONSUMED'
