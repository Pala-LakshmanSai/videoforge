-- Points both hosted text stages at the model their provider still serves, and reopens the stage-3
-- redispatch budget that a provider outage had spent.
--
-- The pinned `deepseek:v4@flash` text backend began failing every dispatch: the account's Runware
-- error ledger recorded two server errors on 2026-09-16 and seven on 2026-09-18 against that model
-- with no accepted result since 2026-09-16T12:41Z, an archived task read answered `taskNotFound`, and
-- the catalog's successor is rejected by the text API as `invalidModel`. The application now pins
-- `google:gemini@3.5-flash` (its contract accepts the same strict JSON Schema request and returned
-- schema-valid JSON), so the execution profiles recorded by these capabilities must name the same
-- model: a profile that records a model the product no longer sends is a false record.
--
-- The stage-3 budget moves from six to ten with the application constant, so a revision that spent
-- its whole budget while the model was dead can continue once the cause is gone. The bound stays a
-- small constant, every redispatch is still separately reserved, and nothing else about either
-- capability changes: a redispatch is still allowed only when the previous attempt produced no
-- accepted result, and the retryable problem-code gates are untouched.
