-- Bind both production activation loaders to the complete current V2-09 migration ledger.
...
BEGIN
  ...
  RETURN jsonb_set(snapshot,'{migrationLedger}',coalesce((SELECT jsonb_agg(jsonb_build_object(
    ...
    WHERE m.version BETWEEN 37 AND 80),'[]'::jsonb));
END; $$;
...
BEGIN
  ...
  SELECT jsonb_agg(jsonb_build_object('version',version,'sha256',sha256) ORDER BY version)
    INTO ledger FROM public.videoforge_schema_migrations WHERE version BETWEEN 37 AND 80;
