-- Retires the mis-hashed text profile revisions and lands revision 9 / 4 with the correct hash.
--
-- Execution profiles are immutable snapshots (the protect trigger refuses both UPDATE and DELETE of a
-- TESTED row), so the revision-8 / revision-3 rows created by 0168 - which carry `configuration_hash`
-- computed from compact JSON instead of the jsonb text form the capabilities hash - cannot be
-- repaired in place. They are retired, and the profile moves to revision 9 (voiceover context) and 4
-- (scene prompts) with the hash computed by the database from the very literal the capability uses.
-- A DO block re-derives both hashes the way the capability does and refuses to commit if they differ.

UPDATE public.execution_profiles
   SET state = 'RETIRED', retired_at = now()
 WHERE (name = 'Hosted Runware voiceover context' AND revision = 8)
    OR (name = 'Hosted Runware scene prompts' AND revision = 3);
...
SELECT gen_random_uuid(),profile.account_id,profile.workspace_id,
       'Hosted Runware voiceover context',9,'PROMPT','TESTED','RUNWARE',
 ...
 CROSS JOIN (SELECT '{"model":"google:gemini@3.5-flash","operation":"voiceover-context-v10","provider":"runware"}'::jsonb AS configuration) cfg
 ON CONFLICT ON CONSTRAINT execution_profiles_workspace_id_name_revision_key DO NOTHING;
INSERT INTO public.execution_profiles(id,account_id,workspace_id,name,revision,lane,state,
  dispatch_target,configuration,configuration_hash,maximum_rate_micro_usd,checked_at,created_at)
SELECT gen_random_uuid(),profile.account_id,profile.workspace_id,
       'Hosted Runware scene prompts',4,'PROMPT','TESTED','RUNWARE',
       cfg.configuration,'sha256:'||encode(sha256(convert_to(cfg.configuration::text,'UTF8')),'hex'),
       40000,now(),now()
  FROM (SELECT DISTINCT account_id,workspace_id FROM public.execution_profiles
         WHERE name='Hosted Runware scene prompts' AND revision=2) profile
 CROSS JOIN (SELECT '{"model":"google:gemini@3.5-flash","operation":"scene-prompt-writer-v2","provider":"runware"}'::jsonb AS configuration) cfg
 ON CONFLICT ON CONSTRAINT execution_profiles_workspace_id_name_revision_key DO NOTHING;
DO $$
DECLARE
  context_hash text;
  prompt_hash text;
  expected_context text := 'sha256:'||encode(sha256(convert_to(('{"model":"google:gemini@3.5-flash","operation":"voiceover-context-v10","provider":"runware"}'::jsonb)::text,'UTF8')),'hex');
  expected_prompt text := 'sha256:'||encode(sha256(convert_to(('{"model":"google:gemini@3.5-flash","operation":"scene-prompt-writer-v2","provider":"runware"}'::jsonb)::text,'UTF8')),'hex');
BEGIN
  SELECT configuration_hash INTO context_hash FROM public.execution_profiles
   WHERE name='Hosted Runware voiceover context' AND revision=9 LIMIT 1;
  SELECT configuration_hash INTO prompt_hash FROM public.execution_profiles
   WHERE name='Hosted Runware scene prompts' AND revision=4 LIMIT 1;
  IF context_hash IS DISTINCT FROM expected_context OR prompt_hash IS DISTINCT FROM expected_prompt THEN
    RAISE EXCEPTION 'hosted text profile hash still disagrees with the capability computation' USING ERRCODE='23514';
  END IF;
END $$;
...
BEGIN
  ...
  INSERT INTO public.execution_profiles(id,account_id,workspace_id,name,revision,lane,state,
    dispatch_target,configuration,configuration_hash,maximum_rate_micro_usd,checked_at,created_at)
  VALUES(profile_id,account_id,workspace_id,'Hosted Runware voiceover context',9,'PROMPT','TESTED',
  ...
  SELECT * INTO existing_profile FROM public.execution_profiles profile
   WHERE profile.account_id=account_id AND profile.workspace_id=workspace_id
     AND profile.name='Hosted Runware voiceover context' AND profile.revision=9 FOR SHARE;
