-- Keep lease admission serialized without reversing revocation lock order.
CREATE OR REPLACE FUNCTION public.videoforge_assert_media_worker_lease_group()
...
BEGIN
  ...
  -- Identity-preserving updates cannot enlarge an active work group. Do not
  -- acquire a device lock after UPDATE has already locked the lease row:
  -- device revocation takes the opposite order (device, then leases).
  IF TG_OP='UPDATE' AND (
    OLD.state IN ('CLAIMED','RUNNING','COMPLETING') OR
    NEW.state NOT IN ('CLAIMED','RUNNING','COMPLETING')
  ) THEN
    RETURN NEW;
  END IF;
