-- Stage 3 now dispatches GPT-5 Nano, while migration 0060 still recorded the retired DeepSeek
-- context profile. Preserve immutable revision 1 history and select a new exact revision 2 profile
...
DECLARE
  ...
  profile_config jsonb:='{"model":"openai-gpt-5-nano","operation":"voiceover-context-v4","provider":"runware"}'::jsonb;
...
BEGIN
  ...
  VALUES(profile_id,account_id,workspace_id,'Hosted Runware voiceover context',2,'PROMPT','TESTED',
  ...
  SELECT * INTO existing_profile FROM public.execution_profiles profile
   WHERE profile.account_id=account_id AND profile.workspace_id=workspace_id
     AND profile.name='Hosted Runware voiceover context' AND profile.revision=2 FOR SHARE;
