-- 0115_hosted_v209_expired_lease_admission.sql
--
-- Fair-queue selection treated any lease row still marked ACTIVE as occupying its account and its
-- slot, without checking whether that lease had already expired. Nothing reclaims a lease whose
-- holder stopped observing, so a single stranded generation permanently blocked every later
-- request on the account: admission returned WAITING forever even with a free slot.
--
-- A lease past expires_at, or already released, no longer holds the account or the slot. Live
-- leases are unaffected, so a genuinely running pair still serialises exactly as before, and the
-- stranded generation keeps its own rows for reconciliation instead of blocking the queue.

CREATE OR REPLACE FUNCTION public.videoforge_admit_hosted_v209_generation(supplied_account_id uuid, supplied_workspace_id uuid, supplied_user_id uuid, supplied_project_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
     ...
     AND NOT EXISTS (
       SELECT 1 FROM public.provider_workload_leases active_lease
        WHERE active_lease.account_id=queued.account_id
          ...
          AND active_lease.expires_at>db_now
     )
     AND NOT EXISTS (
