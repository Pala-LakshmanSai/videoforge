-- Report the run row the claim actually owns.
--
-- The final RETURN of `videoforge_prepare_hosted_prompt_run` reported `run_id`, the id generated for
-- this call. A first dispatch inserts that id, so it matched; an approved redispatch rebinds the row
-- the revision already owns and a different id is in the table. The route therefore settled failures
-- and recorded batch progress against a run id that does not exist, and because the settle threw
-- `hosted prompt failure is invalid` inside a swallowed catch, the run sat in DISPATCHING with no
-- visible cause. Observed live: run 4a57c2ee rebound, settle attempted against 24f67cd7.
...
BEGIN
  ...
  END IF;
  -- Return the run row this call actually owns: a first dispatch created `run_id`, while an
  -- approved redispatch rebound `existing.id`, and every later step (batch progress, the failure
  -- settle, acceptance) addresses the persisted row. Returning the freshly generated id here made a
  -- redispatch report progress and failures against a run that does not exist.
  RETURN jsonb_build_object('created',true,'state','DISPATCHING','run_id',
    CASE WHEN existing.id IS NOT NULL THEN existing.id ELSE run_id END,
