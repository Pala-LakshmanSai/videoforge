-- Scope the reserved-cost idempotency key to the attempt.
--
-- Same 23505 class as 0154, one row further on: `cost_events` keyed the prompt reservation on
-- `workspace_id, idempotency_key='hosted-prompt:<revision>:reserved'`, so an approved redispatch
-- failed with `duplicate key value violates unique constraint "cost_events_workspace_id_idempotency_key_key"`
-- after the task, attempt, and outbox rows had already been made unique.
--
-- `sequence` needs no change: it is allocated as `coalesce(max(sequence),0)+1` for the owner on every
-- dispatch, so a redispatch naturally takes the next number. A first dispatch keeps its exact key.
