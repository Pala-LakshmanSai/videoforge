-- Stage 3 v6 was rejected by the live provider with a generic HTTP 400 even though its request
-- fields are documented. Preserve immutable revisions 1 through 4 and select revision 5 for
-- request v7, restoring the previously accepted low reasoning value with the 3,000-token ceiling.
...
DECLARE
  ...
  profile_config jsonb:='{"model":"openai-gpt-5-nano","operation":"voiceover-context-v7","provider":"runware"}'::jsonb;
...
BEGIN
  ...
  VALUES(profile_id,account_id,workspace_id,'Hosted Runware voiceover context',5,'PROMPT','TESTED',
  ...
  SELECT * INTO existing_profile FROM public.execution_profiles profile
   WHERE profile.account_id=account_id AND profile.workspace_id=workspace_id
     AND profile.name='Hosted Runware voiceover context' AND profile.revision=5 FOR SHARE;
