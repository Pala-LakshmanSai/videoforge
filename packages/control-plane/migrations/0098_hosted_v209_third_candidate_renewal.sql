-- Migration 0098: permit exactly one third append-only renewal for the same V2-09
-- generation while preserving the base candidate and both prior renewals as immutable evidence.
...
ALTER TABLE public.hosted_v209_ordinary_dispatch_candidate_renewals
  ...
  ADD CONSTRAINT hosted_v209_renewal_ordinal_check CHECK(renewal_ordinal IN (1,2,3)),
  ADD CONSTRAINT hosted_v209_renewal_predecessor_check
    ...
    OR (renewal_ordinal IN (2,3) AND previous_candidate_sha256 IS NOT NULL
...
DO $patch_third_candidate_renewal$
DECLARE
...
BEGIN
  ...
  IF target_count<>1
     OR position('(SELECT count(*) FROM public.videoforge_schema_migrations)<>97' IN definition)=0
     OR position('(SELECT max(version) FROM public.videoforge_schema_migrations)<>97' IN definition)=0
     OR position('WHERE migration.version=97' IN definition)=0
     ...
     OR position('renewed_candidate_document,renewed_expires_at,supplied_audit_id,db_now,2,candidate.candidate_sha256,candidate.approval_id)' IN definition)=0
    ...
    RAISE EXCEPTION 'hosted V2-09 third renewal preimage drifted' USING ERRCODE='55000';
  END IF;
  patched:=replace(definition,'(SELECT count(*) FROM public.videoforge_schema_migrations)<>97',
    '(SELECT count(*) FROM public.videoforge_schema_migrations)<>98');
  patched:=replace(patched,'(SELECT max(version) FROM public.videoforge_schema_migrations)<>97',
    '(SELECT max(version) FROM public.videoforge_schema_migrations)<>98');
  ...
  IF patched=definition OR position('migration.version=97' IN patched)>0
     OR position('generation_request_id=request.id)<>1' IN patched)>0
     OR position('candidate:=public.videoforge_effective_hosted_v209_candidate' IN patched)=0
     OR position('db_now,3,candidate.candidate_sha256,candidate.approval_id' IN patched)=0 THEN
    RAISE EXCEPTION 'hosted V2-09 third renewal patch failed' USING ERRCODE='55000';
  END IF;
  EXECUTE patched;
END
$patch_third_candidate_renewal$;
