-- Migration 0112: owner cancellation accepts one exact untouched ordinary OUTBOXED pair.
-- Materializations are immutable construction lineage, not provider dispatch evidence.
-- Tenant-owned cancellation for a hosted video that has not crossed the provider boundary.
--
-- Project archive already refuses active work. This migration extends the matching product
-- capability to one exact untouched OUTBOXED pair: both ordinary lanes may already have their
-- immutable materializations, but neither outbox may have been sent and no provider evidence may
-- exist. Billing and security lineage remain append-only.

CREATE OR REPLACE FUNCTION public.videoforge_cancel_hosted_project_predispatch(
...
DECLARE
  ...
  untouched_pair boolean := false;
  materialization_count integer;
  exact_materialization_count integer;
...
BEGIN
  ...
  IF attempt_count=2 THEN
    SELECT count(*)=2
       AND count(*) FILTER (WHERE attempt.lane='mage_image')=1
       AND count(*) FILTER (WHERE attempt.lane='soulx_avatar')=1
       AND count(*) FILTER (WHERE attempt.state='OUTBOXED'
         AND outbox.state='READY_TO_DISPATCH' AND outbox.send_attempt_count=0)=2
      INTO untouched_pair
      FROM public.serverless_attempts attempt
      LEFT JOIN public.serverless_dispatch_outbox outbox ON outbox.attempt_id=attempt.id
     WHERE attempt.account_id=supplied_account_id
       AND attempt.workspace_id=supplied_workspace_id
       AND attempt.generation_request_id=target_request.id;
    IF NOT coalesce(untouched_pair,false) THEN
      RAISE EXCEPTION 'hosted project provider-safe cancellation unavailable' USING ERRCODE='55000';
    END IF;
  END IF;

  SELECT count(*) INTO materialization_count
    FROM public.hosted_v209_ordinary_lane_materializations materialization
  ...
  SELECT count(*) INTO exact_materialization_count
    FROM public.hosted_v209_ordinary_lane_materializations materialization
    JOIN public.serverless_attempts attempt
      ON attempt.id=materialization.attempt_id
     AND attempt.account_id=supplied_account_id
     AND attempt.workspace_id=supplied_workspace_id
     AND attempt.generation_request_id=target_request.id
     AND attempt.lane=materialization.lane
   WHERE materialization.account_id=supplied_account_id
     AND materialization.workspace_id=supplied_workspace_id
     AND materialization.generation_request_id=target_request.id;
  IF (attempt_count=0 AND materialization_count<>0)
     OR (attempt_count=2 AND (
       materialization_count NOT IN (0,2)
       OR exact_materialization_count<>materialization_count
       OR (materialization_count=2 AND (
         SELECT count(DISTINCT materialization.lane)
           FROM public.hosted_v209_ordinary_lane_materializations materialization
          WHERE materialization.account_id=supplied_account_id
            AND materialization.workspace_id=supplied_workspace_id
            AND materialization.generation_request_id=target_request.id
    ...
    RAISE EXCEPTION 'hosted project ordinary materialization lineage unavailable' USING ERRCODE='55000';
  END IF;
  ...
  SELECT
    ...
    +(SELECT count(*) FROM public.serverless_reconciliations reconciliation
     ...
     WHERE attempt.account_id=supplied_account_id
       AND attempt.workspace_id=supplied_workspace_id
       AND attempt.generation_request_id=target_request.id)
    +(SELECT count(*) FROM public.serverless_provenance_receipts receipt
      JOIN public.serverless_attempts attempt ON attempt.id=receipt.attempt_id
     WHERE attempt.account_id=supplied_account_id
       AND attempt.workspace_id=supplied_workspace_id
       AND attempt.generation_request_id=target_request.id)
    +(SELECT count(*) FROM public.hosted_pair_cleanup_observations observation
      JOIN public.serverless_attempts attempt ON attempt.id=observation.attempt_id
     WHERE attempt.account_id=supplied_account_id
       AND attempt.workspace_id=supplied_workspace_id
       AND attempt.generation_request_id=target_request.id)
    +(SELECT count(*) FROM public.hosted_serverless_output_barrier_completions completion
      JOIN public.serverless_attempts attempt ON attempt.id=completion.attempt_id
     WHERE attempt.account_id=supplied_account_id
       AND attempt.workspace_id=supplied_workspace_id
       AND attempt.generation_request_id=target_request.id)
  ...
  IF EXISTS (SELECT 1 FROM public.provider_workload_leases lease
     ...
     OR EXISTS (SELECT 1 FROM public.serverless_dispatch_outbox outbox
      JOIN public.serverless_attempts attempt ON attempt.id=outbox.attempt_id
      WHERE attempt.generation_request_id=target_request.id
        AND (outbox.state<>'DEAD_LETTER' OR outbox.send_attempt_count<>0))
