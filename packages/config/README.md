...
`execution-profile-catalog.v1.json` is the UI-facing catalog for the two primary parallel lanes. Each lane exposes one selectable immutable profile named `Fixture`; it has no provider endpoint, GPU assignment, or external spend. The documented Mage and EchoMimicV3-Flash GPU candidates are disabled `BENCHMARK_REQUIRED` metadata tied to `GATE_GPU_001`. They are planning priorities, never availability claims.
