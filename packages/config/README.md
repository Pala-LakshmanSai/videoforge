...
`execution-profile-catalog.v1.json` is the UI-facing catalog for the two primary parallel lanes. Each lane exposes one selectable immutable profile named `Fixture`; it has no provider endpoint, GPU assignment, or external spend. Catalog selection remains blocked by `GATE_SERVERLESS_CONTRACT_001`. The exact Mage-Flow Turbo INT8 ConvRot and SoulX-FlashHead Pro RTX 4090 candidates are disabled `BENCHMARK_REQUIRED` metadata tied to `GATE_SERVERLESS_MAGE_001` and `GATE_SERVERLESS_SOULX_001`, respectively. They are qualification targets, never availability claims or GPU fallback lists.
