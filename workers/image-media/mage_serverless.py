...
from secure_scratch import ScratchIsolationError, mage_worker_io
...
def _authority_expectations(envelope: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        "expected_account_id": envelope["tenant"]["account_id"],
        "expected_workspace_id": envelope["tenant"]["workspace_id"],
        "expected_deployment_id": runtime["deployment_id"],
...
async def handler(job: dict[str, Any]) -> dict[str, Any]:
    """Never retries or redelivers itself; the control plane owns duplicate reconciliation."""
    try:
        ...
        input_ports = tuple(ports.get("inputs", ()))
        output_ports = tuple(ports.get("outputs", ()))
        if len(output_ports) != len(mage_job.items):
            raise ServerlessMageError("MAGE_SERVERLESS_OUTPUT_PORT_COUNT_INVALID")
        reservation_ids = {port.get("reservation_id") for port in output_ports if isinstance(port, dict)}
        if reservation_ids != set(accepted["artifacts"]["transfer_port_reservation_ids"]):
